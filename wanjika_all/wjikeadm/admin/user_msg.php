...
//-- 批量导入用户
/*------------------------------------------------------ */
elseif ($_REQUEST['act']=='patch_user_by_mobile')
{
	$moblist = explode("\n",$_POST['mobile_list']);
	$moblist = array_unique($moblist);
	if ($_POST['mobile_list']!="") {
		$smarty->assign('posted',    1);
		$show = "导入成功！<br>其中异常手机号如下：";
		while (list($key,$mobile)=each($moblist)) {
			$mobile = trim($mobile);
			if(preg_match("/1\d{10}/",$mobile))
			{
				...
				$user_name = $mobile;
				$mobile_phone = $mobile;
				$sql = "INSERT INTO ".$ecs->table('users')." (user_name, email, password, mobile_phone ) ".
	...
	$smarty->display('patch_user_by_mobile.htm');
}
