...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'download')
{
    ...
    $order_brush = (isset($_REQUEST['order_brush'])) ? intval($_REQUEST['order_brush']) : -1;

    /* 获取访问购买的比例数据 */
    $click_sold_info = click_sold_info($cat_id, $brand_id, $show_num,$order_brush);
    ...
    $smarty->assign('order_brush', $_REQUEST['order_brush']);
    ...
    $filename = 'visit_sold';
    $smarty->assign('action_link',  array('text' => $_LANG['download_visit_buy'], 'href' => 'visit_sold.php?act=download&show_num=' . $show_num . '&cat_id=' . $cat_id . '&brand_id=' . $brand_id . '&show_num=' . $show_num . '&order_brush=' . $order_brush ));
 ...
 */
 function click_sold_info($cat_id, $brand_id, $show_num,$order_brush)
 {
    global $db, $ecs;
    ...
    if ($order_brush >= 0)
    {
        $where .= " AND o.order_brush = '$order_brush' ";
    }
