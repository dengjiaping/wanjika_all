...
elseif($_REQUEST['act'] == 'act_export_ecshop')
{
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        //商品描述中的图片
        preg_match_all("/src=\'\S+\/(images\S+)\'/", $goods_value['goods_desc'], $matches);

        if (!empty($matches) && !empty($matches[1]))
        {
            foreach ($matches[1] as $img_url)
            {
                $zip->add_file(file_get_contents(ROOT_PATH . $img_url), $img_url);
            }
        }
    ...
    //商品相册
    $sql = "SELECT gg.* FROM " . $ecs->table('goods') . ' AS g LEFT JOIN ' . $ecs->table('goods_gallery') . ' AS gg ON g.goods_id = gg.goods_id ' . $where;
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
        !empty($row['img_url']) && $zip->add_file(file_get_contents(ROOT_PATH . $row['img_url']), $row['img_url']);
        !empty($row['thumb_url']) && $zip->add_file(file_get_contents(ROOT_PATH . $row['thumb_url']), $row['thumb_url']);
        !empty($row['img_original']) && $zip->add_file(file_get_contents(ROOT_PATH . $row['img_original']), $row['img_original']);
    }
