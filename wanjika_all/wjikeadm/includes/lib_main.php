...
function assign_dynamic($tmp)
{
    ...
    $artcat_names = get_artcatname();
    $cat_names = get_catname();
   ...
   foreach ($res AS $row)
    {
        switch ($row['type'])
        {
            case 1:
                /* 分类下的商品 */
                $assign_cat_goods = assign_cat_goods($row['id'], $row['number'], $from = 'web', $order_rule = '', $flag = false);
                $assign_cat_goods['name'] = (($cat_names[$row['id']] == null ? '': $cat_names[$row['id']]));
                $GLOBALS['smarty']->assign('goods_cat_' . $row['id'], $assign_cat_goods);
            break;
            case 2:
                /* 品牌的商品 */
            ...
            case 3:
                /* 文章列表 */
                $cat_articles = assign_all_articles($row['id'], $row['number'], $artcat_names);
...
function assign_all_articles($id, $num, $artcat_names)
{
    $cat['id']       = $id;
    $cat['name']     = $artcat_names[$id];
    $cat['url']      = build_uri('article_cat', array('acid' => $id), $cat['name']);
