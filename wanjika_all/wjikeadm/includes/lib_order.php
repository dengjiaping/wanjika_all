 ...
 * @return  float
 */
function pay_fee($payment_id, $order_amount, $cod_fee=null, $mobile = '')
{
	$pay_fee = 0;
	$payment = payment_info($payment_id);
	$rate    = ($payment['is_cod'] && !is_null($cod_fee)) ? $cod_fee : $payment['pay_fee'];

    if ($payment['pay_code'] == 'umpay')
    {
...
//		$max_amount      -= $use_bonus; // 积分最多还能支付的金额
		//		update_gift_card($order['gift_id'],$use_gift);
	}
	...
	{
        $mobile = $GLOBALS['db']->getOne("SELECT  mobile_phone FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '" . $_SESSION['user_id'] . "' ");
		$total['pay_fee']      = pay_fee($order['pay_id'], $total['amount'], $shipping_cod_fee, $mobile);
	}
 ...
 * @return  bool
 */
function update_gift_card($gift_id,$use_gift)
{
	$sql = "UPDATE " . $GLOBALS['ecs']->table('gift_card') .
	" SET leave_money=leave_money-$use_gift WHERE card_id = '" . $gift_id ."'";
...
function user_available_bonus($user_id, $cart_goods)
{
	...
	$sql = "SELECT t.type_id, t.type_name, t.type_money, t.min_goods_amount, t.use_goods_ids,b.bonus_id, t.pay_ids " .
	"FROM " . $GLOBALS['ecs']->table('bonus_type') . " AS t," .
	$GLOBALS['ecs']->table('user_bonus') . " AS b " .
	"WHERE t.type_id = b.bonus_type_id " .
	"AND t.use_start_date <= '$today' " .
	...
	if (!empty($bonus_list_distinct))
	{
		foreach ($bonus_list_distinct as $key => $info)
		{
			...
			{
				//if (in_array($goods['goods_id'], $applies_goods_ids))
                if (strpos($info['use_goods_ids'], $goods['goods_id']) !== false)
				{
					$applies_goods_price += $goods['goods_price'] * $goods['goods_number'];
				}
			}
			if ($applies_goods_price < $info['min_goods_amount'])
			{
...
function user_gift($user_id, $goods_amount = 0)
{
	...
	$sql = "SELECT a.card_id,a.card_sn,a.leave_money,a.card_password,b.goods_name from ecs_gift_card as a,ecs_goods as b where a.member_id='".$user_id."' and a.end_date>='".$today."' and leave_money-useing_money>0 and a.goods_id=b.goods_id and card_status=1";
	return $GLOBALS['db']->getAll($sql);
}
...
/**
 * 检查礼品卡是否已使用
 * @param   int $card_id   礼品卡id
 * @return  bool
 */
function gift_used($card_id)
{
	$sql = "SELECT card_id FROM " . $GLOBALS['ecs']->table('gift_card') .
	" WHERE card_id = '$card_id'";
...
function use_gift( $order,$total)
{
	...
	update_order($order['order_id'], array( 'gift_card_id' => "{$order['gift_id']}",'gift_money' => "{$total['gift']}"));
...
function get_total_bonus()
{
	...
	/* 按订单发的红包 */
	$sql = "SELECT sum(FLOOR('$amount' / min_amount) * type_money) " .
...
- $order['money_paid'];
	...
	if ($order['pay_id'] > 0)
	{
        ...
        $mobile = $GLOBALS['db']->getOne("SELECT  mobile_phone FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '" . $_SESSION['user_id'] . "' ");
		$order['pay_fee'] = pay_fee($order['pay_id'], $order['order_amount'], $cod_fee, $mobile);
    ...
    $file_name = "/web/pay_log/merge_order_".date("Ymd").".log";
    $logStr = "from=$from_order_sn|to=$to_order_sn|time=" .date('Y-m-d H:i:s') . "\n";
 ...
 * @return  bool
 */
function send_order_bonus($order_id, $pay_name = '', $bank_abbr = '')
{
	/* 取得订单应该发放的红包 */
	...
	if ($bonus_list)
	{
		...
		foreach ($bonus_list AS $bonus)
		{
            ...
            if ($bonus['type_id'] == 34 && $pay_name == '在线支付' && $bank_abbr == 'CCB')
            {
                //nothing
            }
            else
            {
                continue;       //在线支付选择建行支付的发放特定优惠券，否则不发放
            }
			/* 修改用户红包 */
...
function get_province_by_mobile($mobile)
{
    $url = "http://tcc.taobao.com/cc/json/mobile_tel_segment.htm?tel=$mobile";

    $res = file_get_contents($url);

    preg_match("/ province:\'(\S*)\'/", $res, $matches);

    return iconv('gbk', 'UTF-8', $matches[1]);
}

function get_info_by_mobile($mobile)
{
    $url = "http://tcc.taobao.com/cc/json/mobile_tel_segment.htm?tel=$mobile";

    $res = file_get_contents($url);
    preg_match("/ province:\'(\S*)\'/", $res, $province);
    preg_match("/ catName:\'(\S*)\'/", $res, $catName);
    $arr = array();
    $arr['province'] = iconv('gbk', 'UTF-8', $province[1]);
    $arr['catName'] = iconv('gbk', 'UTF-8', $catName[1]);
 ...
 * 获得商品所在分类
 */
function get_goodscategory($goods_id)
{
    $cat_name = '';

    $sql = "SELECT c.cat_name FROM " .$GLOBALS['ecs']->table('goods') .
        " AS g INNER JOIN " . $GLOBALS['ecs']->table('category') . " AS c ON g.cat_id = c.cat_id WHERE g.goods_id = '" . $goods_id . "' ";
    $r = $GLOBALS['db']->getAll($sql);
    if(count($r) > 0){
        $cat_name = $r[0]['cat_name'];
    }
...
{
    $result = array();
    $result['reg_time'] = '2012-12-12';

    $sql = "SELECT email,mobile_phone,reg_time FROM " .$GLOBALS['ecs']->table('users') .
        " WHERE user_id = '" . $user_id . "' ";
    $r = $GLOBALS['db']->getAll($sql);
    if(count($r) > 0){
        $result['reg_time'] = date("Y-m-d",$r[0]['reg_time']);
        $result['phone'] = $r[0]['mobile_phone'];
