 ...
 * @return  void
 */
function order_paid($log_id, $pay_status = PS_PAYED, $note = '', $bank_abbr = '')
{
    /* 取得支付编号 */
    ...
    if ($log_id > 0)
    {
        ...
        if ($pay_log && $pay_log['is_paid'] == 0)
        {
            ...
            {
                /* 取得订单信息 */
                $sql = 'SELECT order_id, user_id, order_sn, pay_name, consignee, address, tel, mobile, shipping_id, extension_code, extension_id, goods_amount ' .
                ...
                $pay_source = $order['pay_name'];
                if ($order['pay_name'] == '移动话费支付')
                {
                    $pay_mobile = $GLOBALS['db']->getOne("SELECT  mobile_phone FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '" . $order['user_id'] . "' ");
                    $pay_source = @get_province_by_mobile($pay_mobile);
                }
                ...
                send_order_bonus($order_id, $order['pay_name'], $bank_abbr);
                ...
                if (!empty($virtual_goods) && key($virtual_goods) != 'charge_calls')
                {
                    $msg = '';
                ...
                elseif(key($virtual_goods) == 'charge_calls'){
                    $sql = "SELECT keywords FROM ".$GLOBALS['ecs']->table('goods') ." WHERE goods_id = ".$virtual_goods['charge_calls'][0]["goods_id"];
                    $charge_amount = $GLOBALS['db']->getOne($sql);
                    $virtual_goods['charge_calls'][0]["tel"] = $order['mobile'];
                    $virtual_goods['charge_calls'][0]["charge_amount"] = $charge_amount;

                    if (!charge_goods_ship($virtual_goods,$msg, $order_sn, true))
                    {
                        $GLOBALS['_LANG']['pay_success'] .= '<div style="color:red;">'.$msg.'</div>'.$GLOBALS['_LANG']['charge_calls_fail'];
                    }
                }
...
//根据手机号判断调用哪个商户号和私钥
function get_merid_and_primary_key($mobile)
{
    ...
    $merId = 6877;
    $primary_key = '6877_WanJiKe.key.pem';
    preg_match("/ province:\'(\S*)\'/", $res, $matches);
    if (iconv('gbk', 'UTF-8', $matches[1]) == '北京')
    {
        $merId = 7145;
        $primary_key = '7145_WanJiKe.key.pem';
    }
    else if (iconv('gbk', 'UTF-8', $matches[1]) == '上海')
    {
        $merId = 6844;
        $primary_key = '6844_WanJiKe.key.pem';
    }

    return array('merId' => $merId, 'primary_key' => $primary_key);
}
