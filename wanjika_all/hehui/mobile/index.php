...
//首页广告
$uri = $ecs->url();
$uri = str_replace('/mobile', '', $uri);
$player_db = get_flash_xml();
foreach ($player_db as $key => $val)
{
    if (strpos($val['src'], 'http') === false)
    {
        $player_db[$key]['src'] = $uri . $val['src'];
        $pos = strpos($val['url'], '/', 10);
        $player_db[$key]['url'] = substr($val['url'], 0, $pos) . '/mobile/' . substr($val['url'], $pos + 1);
        $player_db[$key]['url'] = str_replace('?id=', '?c_id=', $player_db[$key]['url']);
    }
}

$smarty->assign('index_ad', $player_db);
$smarty->assign('ad_num', count($player_db));
$smarty->assign('ad_width', count($player_db) * 100);
