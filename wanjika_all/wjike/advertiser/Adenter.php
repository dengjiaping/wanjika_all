<?php
	 ...
	 */
	class Adenter{
		private $DEFAULT_CAMPAIGN_ID = "18432";					// 活动id 即cid，上线时需要设置亿起发提供的cid
		private $DEFAULT_TARGET 	 = "http://www.wjike.com"; 	 // 着陆页面，默认是首页
		private $DEFAULT_CHANNEL     = "cps";					// 业务合作方式
		public 	$UNION_COOKIE_NAME 	 = "yiqifa";				// 保存的cookie名
		private $UNION_COOKIE_DOMAIN = ".wjike.com"; 			// 替换为自己网站的域
		...
		function jump($source,$channel,$campagin_id,$yiqifa_wi,$target_url){	
			...
			} else if (strcmp(substr($target_url, 0,7),"http://")!=0
					&& !strcmp(substr($target_url, 0,8),"https://")!=0) { // 如果目标地址不是以“http://”或“https://”开头，则加上"http://"
