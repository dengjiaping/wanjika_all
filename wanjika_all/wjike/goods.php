...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'select_goods')
{
    include('includes/cls_json.php');

    $json   = new JSON;
    $res    = array('error' => '1', 'msg' => '');
    $collect = $_REQUEST['collect_value'];
    $collect = explode(',',$collect);
    sort($collect);
    $collect = implode(',',$collect);
    $r = get_goodsid_by_cattr($collect);
    if($r != "")
    {
        $url = 'http://item.wjike.com/'.$r.'.html';
        if(is_local()){
            $url = build_uri('goods', array('gid' => $r));
        }
        $r = $url;
        $res['error'] = '0';
    }
...
if (!$smarty->is_cached('goods.dwt', 1))
{
    ...
    {
        if($goods['overseas_logo'] == $value['country_code'])
        {
            $goods['overseas_logo_name'] = $value['country_name'];
        }
    }
    if ($goods === false)
    {
    ...
    else
    {
        ...
        //获取商品规格
        $goods_collect = get_goods_collectvalues($goods['goods_id']);
        foreach($goods_collect AS $key=>$value)
        {
            if($goods_collect[$key]['goods_attrid'] == null || $goods_collect[$key]['goods_attrid'] == '')
            {
                unset($goods_collect[$key]);
            }
        }
        $smarty->assign('goods_collect',              $goods_collect);
        //广告投放参数
        $goods['cate_name'] = get_cat_name($goods['cat_id']);
        $goods['ad_goods_img'] = 'http://www.wjike.com/'.$goods['original_img'];
        $goods['ad_goods_url'] = 'http://www.wjike.com/goods.php?id='.$goods['goods_id'];
...
//        $smarty->assign('description',        htmlspecialchars($goods['goods_brief']));
        /* seo优化 */
        $keywords = $goods['goods_name'].'优惠，'.$goods['goods_name'].'价格，'.$goods['goods_name'].'品牌';
        $description = '万集客为您提供'.$goods['goods_name'].'的价格，'.$goods['goods_name'].'的优惠，'.$goods['goods_name'].'的品牌。万集客会员制商城为您提供优质的进口食品，进口美妆个护，进口母婴用品，家居家电，手机数码。';
...
if($_SESSION['user_id'] >0)
{
    ...
    {
        $smarty->assign('collect_goods',  true);
    }
}
