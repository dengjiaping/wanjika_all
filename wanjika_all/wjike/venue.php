...
$cache_id = sprintf('%X', crc32($_SESSION['user_rank'] . '-' . $_CFG['lang'] . '-' . $venue_id));
...
if (!$smarty->is_cached($templates, $cache_id))
{
    ...
    foreach ($venue AS $key=>$value)
    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
            ...
            'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb ,g.original_img ' .
    ...
    $smarty->assign('ur_here',          $position['ur_here'] . '> ' . $venue['title']);     // 当前位置
...
//    $smarty->assign('title_pic',        $venue['title_pic']);      // 分类标题图片地址
//    $smarty->assign('base_style',       '#' . $venue['base_style']);     // 基本风格样式颜色

    $template_file = empty($venue['template']) ? 'venue.dwt' : $venue['template'];
}
/* 显示模板 */
