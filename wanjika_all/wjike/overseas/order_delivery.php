...
if (!is_numeric($MerchantOrderID) || !is_numeric($ShipTypeID) || null == $TrackingNumber || !is_numeric($CommitTime)){
    $result['Code']="-1";
...
$sign = $_POST['sign'];//验证码
if($sign != md5($get."25d4ab3495e44bf4aabd55e199e29e3d") ){
    $result['Code']="-2";
...
if($res)
{
    ...
    if($orders_status["user_id"] != 1406833){
        ...
        $mr = $msgsend->send($orders_status["tel"],$msg,"kjt");
    }
...
//        $CommitTime=local_strtotime($CommitTime);
    ...
    function ecord_log($result)
    {
        ...
        $file_name = "/web/kjt_log/kjtorder_log_".date("Ymd") . ".log";
