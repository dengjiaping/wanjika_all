...
if ($_REQUEST['act'] == 'list')
{
    ...
    if ($_REQUEST['download']=='下载') {
        ...
        $str_down = "用户账号\t手机号码\t注册日期\t是否是会员\t会员到期日";

        $list = user_list();
        while ( list ( $key, $val ) = each ( $list['user_list'] ) ) {
            $members_deadline = local_date('Y-m-d', $val ['members_deadline']);
            $str_down .= "\n" . $val ['user_name'] . "\t". $val['mobile_phone'] . "\t" . $val ['reg_time']. "\t" . $val['is_members'] . "\t" . $members_deadline;
        }
...
//-- 添加VIP会员配置
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'members_add')
{
    /* 检查权限 */
    admin_priv('users_manage');

    $members_id = isset($_REQUEST['members_id']) ? trim($_REQUEST['members_id']) : '';

    if(!empty($members_id)){
        $sql = 'SELECT * FROM ' . $ecs->table('user_members') . " WHERE id=$members_id";
        $r = $db->getRow($sql);
        if($r){
            $smarty->assign('members', $r);
        }
    }
    /* 取出注册扩展字段 */
    $sql = 'SELECT goods_id,goods_name FROM ' . $ecs->table('goods') . " WHERE extension_code='goods_members'";
    $r = $db->getAll($sql);
    foreach($r as $v){
        $goods_members_list[$v['goods_id']] = $v['goods_name'];
    }
    $smarty->assign('goods_members_list', $goods_members_list);

    $smarty->assign('ur_here',          $_LANG['11_user_addmembers']);
    $smarty->assign('action_link',      array('text' => $_LANG['12_user_memberslist'], 'href'=>'users.php?act=members_list'));

    $smarty->display('members_info.htm');
}
...
elseif ($_REQUEST['act'] == 'members_list')
{
    /* 检查权限 */
    admin_priv('users_manage');

    $smarty->assign('ur_here',      $_LANG['12_user_memberslist']);
    $smarty->assign('action_link',  array('text' => $_LANG['11_user_addmembers'], 'href'=>'users.php?act=members_add'));

    $members_list = members_list();

    $smarty->assign('members_list',    $members_list['members_list']);
    $smarty->assign('filter',       $members_list['filter']);
    $smarty->assign('record_count', $members_list['record_count']);
    $smarty->assign('page_count',   $members_list['page_count']);
    $smarty->assign('full_page',    1);

    assign_query_info();
    $smarty->display('members_list.htm');
}

/*------------------------------------------------------ */
//-- 添加VIP会员配置
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'insert_members')
{
    /* 检查权限 */
    admin_priv('users_manage');
    $id = empty($_POST['id']) ? 0 : intval($_POST['id']);
    $level = empty($_POST['level']) ? 0 : intval($_POST['level']);
    ...
    switch ($level)
    {
        case 1:     $levelname = '一级会员'; break;
        case 2:     $levelname = '二级会员'; break;
        case 3:     $levelname = '三级会员'; break;
        default:    $levelname = '';
    }
    switch ($type)
    {
        case 1:     $days = 90; break;
        case 2:     $days = 180; break;
        case 3:     $days = 360; break;
        default:    $days = 90;
    }


    if ($level <= 0 || $type <= 0 || $price <= 0 || $goods_id <= 0)
    {
        sys_msg('参数错误', 1);
    }

    /* 提示信息 */
    $link[] = array('text' => $_LANG['go_back'], 'href'=>'users.php?act=members_list');
    $msg = '操作成功';

    //会员配置如果已经关联该商品，则无法继续关联
    if ($db->getOne('SELECT count(*) FROM ' . $ecs->table('user_members') . "  WHERE goods_id = '$goods_id'"))
    {
        sys_msg('该会员商品已被关联', 0, $link);
    }

    if($id > 0){
        $sql = 'UPDATE ' . $ecs->table('user_members') . " SET `level`='$level', `levelname`='$levelname', `type`='$type'".
            ", `days`='$days', `price`='$price', `goods_id`='$goods_id' WHERE `id`='$id'";
        $r = $db->query($sql);
        if(!$r){
            $msg = '操作失败';
        }
        ...
        $sql = 'INSERT INTO '. $ecs->table('user_members') . " (`level`, `levelname`, `type`, `days`, `price`, `goods_id`) ".
            "VALUES ('$level', '$levelname', '$type', '$days', '$price', '$goods_id')";
        $r = $db->query($sql);
        if(!$r){
            $msg = '操作失败';
        }
...
{
    $smarty->assign('ur_here',      $_LANG['13_user_memberintegral']);
    ...
    assign_query_info();
    $smarty->display('members_integral.htm');
}

/*------------------------------------------------------ */
//--       修改积分倍数
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'update_integral')
{
    $members_id       = intval($_POST['id']);
    $integral        = sprintf("%.1f",$_POST['val']);
    $sql="UPDATE ".$ecs->table('user_members'). "SET integralrate='$integral' WHERE id= '".$members_id."'";
    if($db->query($sql))
    {
        make_json_result(stripcslashes($integral));
    }
    else
    {
...
//--       签到配置后台
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'sign_config')
{
    $smarty->assign('ur_here',      $_LANG['14_user_sign_config']);

    $sql = "SELECT * FROM ".$ecs->table('integral');
    $rs = $db->getAll($sql);
    $smarty->assign('integral',    $rs[0]);

    assign_query_info();
    $smarty->display('sign_config.htm');
}

/*------------------------------------------------------ */
//--       编辑签到配置后台
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'insert_integral')
{
    $integral['basic_integral'] = intval($_POST['basic_integral']) > 0 ? intval($_POST['basic_integral']) : 1;
    $integral['day'] = intval($_POST['day']) > 0 ? intval($_POST['day']) : 1;
    $integral['reward_integral'] = intval($_POST['reward_integral']) > 0 ? intval($_POST['reward_integral']) : 1;
    $integral_id = $_POST['integral_id'];
    if($integral_id > 0)
    {
        $db->autoExecute($ecs->table('integral'), $integral, 'UPDATE', "integral_id = '$integral_id'");
    }
    else
    {
        $db->autoExecute($ecs->table('integral'), $integral, 'INSERT');
    }

    /* 提示信息 */
    $link[] = array('text' => $_LANG['go_back'], 'href'=>'users.php?act=sign_config');
    sys_msg($_LANG['integral_success'], 0, $link);
}
/**
 *  返回用户列表数据
 *
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time,members_deadline, is_members ".
    ...
    for ($i=0; $i<$count; $i++)
    {
        ...
        if($user_list[$i]['is_members']){
            $user_list[$i]['is_members'] = '是';
        }
        else{
            $user_list[$i]['is_members'] = '否';
        }
    }
...
function members_list()
{
    ...
    if ($result === false)
    {
        $filter['record_count'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('user_members'));

        /* 分页大小 */
        $filter = page_and_size($filter);
        $sql = "SELECT um.*,g.goods_name ".
            " FROM " . $GLOBALS['ecs']->table('user_members') .
            " AS um inner join " .$GLOBALS['ecs']->table('goods') .
            " AS g on um.goods_id=g.goods_id LIMIT " . $filter['start'] . ',' . $filter['page_size'];
    ...
    $members_list = $GLOBALS['db']->getAll($sql);

    $arr = array('members_list' => $members_list, 'filter' => $filter,
        'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
