...
function SelectZone()
{
  ...
  this.createOptions = function(obj, arr)
  {
    ...
    /**
     * 添加选中项-供红包选择可用商品，可用类使用
     * @param   boolean  all
     * @param   string   id
     */
    this.addItem4Bonus = function(all)
    {
        if (!this.check())
        {
            return;
        ...
        for (var i = 0; i < this.sourceSel.length; i ++ )
        {
            ...
            if (this.targetSel.length > 0)
            {
                ...
                {
                    var opt   = document.createElement("OPTION");
                    opt.value = this.sourceSel.options[i].value;;
                    opt.text  = this.sourceSel.options[i].text;
    ...
    /**
     * 删除选中项-供红包选择可用商品，可用类使用
     * @param   boolean    all
     */
    this.dropItem4Bonus = function(all)
    {
        if (!this.check())
        {
            return;
        ...
        for (var i = this.targetSel.length - 1; i >= 0 ; i -- )
        {
            if (this.targetSel.options[i].selected || all)
            {
                this.targetSel.options.remove(i);
            }
        }
    }
}
