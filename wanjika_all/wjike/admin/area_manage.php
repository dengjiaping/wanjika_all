...
elseif ($_REQUEST['act'] == 'edit_a_name')
{
    check_authz_json('area_manage');
    ...
    if (empty($region_name))
    {
        make_json_error($_LANG['area_name_empty']);
    }
    ...
    /* 查看区域是否重复 */
    $parent_id = $exc->get_name($id, 'parent_id');
    if (!$exc->is_only('area_name', $region_name, $id, "parent_id = '$parent_id'"))
    {
        make_json_error($_LANG['area_name_exist']);
    }

    if ($exc->edit("area_name = '$region_name'", $id))
    {
        admin_log($region_name, 'edit', 'area');
        make_json_result(stripslashes($region_name));
    }
...
elseif ($_REQUEST['act'] == 'edit_a_id')
{
    ...
    if (empty($region_name))
    {
        make_json_error($_LANG['area_id_empty']);
    }
    ...
    /* 查看区域是否重复 */
//    $parent_id = $exc->get_name($id, 'parent_id');
//    if (!$exc->is_only('area_id', $region_name, $id, "parent_id = '$parent_id'"))
//    {
//        make_json_error($_LANG['area_id_exist']);
//    }

    if ($exc->edit("area_id = '$region_name'", $id))
    {
        admin_log($region_name, 'edit', 'area');
        make_json_result(stripslashes($region_name));
    }
