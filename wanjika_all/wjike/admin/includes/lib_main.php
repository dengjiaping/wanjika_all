 ...
 */
function get_goods_list($filter,$is_on_sale=false)
{
    ...
    if($is_on_sale)
    {
        ...
        $sql = 'SELECT goods_id, goods_name, shop_price '.
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . $where . ' AND (is_on_sale=1 OR is_on_sale=2) ' .
            'LIMIT 500';
    }
    else
    {
        /* 取得数据 */
        $sql = 'SELECT goods_id, goods_name, shop_price '.
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . $where . ' AND is_on_sale=1 ' .
            'LIMIT 500';
    }
...
function get_where_sql($filter)
{
    ...
    $where .= isset($filter->gcid) ? ' AND gcid = 0' : '';
...
function warehouse_exists($warehouse,$warehousetype)
{
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('goods_supplier').
        " WHERE warehouse_type = '" . $warehouse . "' AND supplier_type='".$warehousetype ."'";
    return ($GLOBALS['db']->getOne($sql) > 0) ? true : false;
}
/**
 * 获取当前管理员信息
...
/**
 * 检查erp分类名称、分类编码是否已经存在
 * @param   string     $erpcat_name      erp分类名称
 * @param   string     $erpcat_code      erp分类编码
 * @return  array
 */
function erpcat_exists($erpcat_name, $erpcat_code)
{
    if(!empty($erpcat_name))
    {
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('erp_category').
            " WHERE erpcat_name = '$erpcat_name'";
        if($GLOBALS['db']->getOne($sql)>0)
        {
            return true;
        }
    }
    if(!empty($erpcat_code))
    {
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('erp_category').
            " WHERE erpcat_code = '$erpcat_code'";
        if($GLOBALS['db']->getOne($sql)>0)
        {
            return true;
        }
...
/**
 * 检查场馆名称是否已经存在
 * @param   string     $venue_name      erp分类名称
 * @return  array
 */
function venue_exists($venue_name)
{
    if(!empty($venue_name))
    {
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('venue_manage').
 ...
 * @return  array
 */
function warehouse_type_exists($warehouse_type)
{
...
/**
 * 检查商品国家是否已经存在
 * @param   string     $country_name      商品国家
 * @param   string     $country_code      商品国家编号
 * @return  array
 */
function country_type_exists($country_name,$country_code)
{
    if(!empty($country_name))
    {
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('goods_country').
