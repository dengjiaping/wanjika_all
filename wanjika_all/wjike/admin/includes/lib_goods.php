...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $filter['kjt_goodsid']          = empty($_REQUEST['kjt_goodsid']) ? '' : trim($_REQUEST['kjt_goodsid']);
...
//        $where = $filter['cat_id'] > 0 ? " AND " . get_children($filter['cat_id']) : '';
        ...
        /* kjt商品ID */
        if (!empty($filter['kjt_goodsid']))
        {
            $where .= " AND kjt_goods_id LIKE '%" . mysql_like_quote($filter['kjt_goodsid']) . "%'";
        }
        ...
        {
            $where .= " AND supplier_id='$filter[supplier_id]'";
        }
        ...
        if ($filter['is_overseas'] != -1)
        {
            ...
            $where .= " AND is_overseas = '$filter[is_overseas]'";
        }
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('goods'). " AS g LEFT JOIN " . $GLOBALS['ecs']->table('goods_supplier') . " AS s ON g.supplier_id=s.type_id WHERE is_delete='$is_delete' $where";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, warn_number, integral, " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote,s.warehouse_type,s.is_overseas ".
                    " FROM " . $GLOBALS['ecs']->table('goods') . " AS g LEFT JOIN " . $GLOBALS['ecs']->table('goods_supplier') . " AS s ON g.supplier_id=s.type_id WHERE is_delete='$is_delete' $where" .
