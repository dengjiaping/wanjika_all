...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $desc_img_name = basename($image->upload_image($_FILES['brand_desc_img'],'branddescimg'));
    ...
    $sql = "INSERT INTO ".$ecs->table('brand')."(brand_name, site_url, brand_desc, brand_logo, is_show, sort_order,brand_desc_img,overseas_logo) ".
           "VALUES ('$_POST[brand_name]', '$site_url', '$_POST[brand_desc]', '$img_name', '$is_show', '$_POST[sort_order]','$desc_img_name','$_POST[overseas_logo]')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $sql = "SELECT brand_id, brand_name, site_url, brand_logo, brand_desc, brand_logo, is_show, sort_order,brand_desc_img,overseas_logo ".
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    $img_name = basename($image->upload_image($_FILES['brand_logo'],'brandlogo'));
    $desc_img_name = basename($image->upload_image($_FILES['brand_desc_img'],'branddescimg'));
    $param = "brand_name = '$_POST[brand_name]',  overseas_logo='$_POST[overseas_logo]',  site_url='$site_url', brand_desc='$_POST[brand_desc]', is_show='$is_show', sort_order='$_POST[sort_order]' ";
    if (!empty($img_name))
    {
        //有图片上传
        $param .= " ,brand_logo = '$img_name' ";
    }
    if (!empty($desc_img_name))
    {
        //有描述图片上传
        $param .= " ,brand_desc_img = '$desc_img_name' ";
    }
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if (!empty($logo_name))
    {
        ...
        /* 删除该品牌的介绍图 */
    $sql = "SELECT brand_desc_img FROM " .$ecs->table('brand'). " WHERE brand_id = '$id'";
    $desc_name = $db->getOne($sql);
    if (!empty($desc_name))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/branddescimg/' .$desc_name);
    }
...
elseif ($_REQUEST['act'] == 'drop_desc_img')
{
    ...
    if (!empty($logo_name))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/branddescimg/' .$logo_name);
