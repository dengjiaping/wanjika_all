 ...
 * ============================================================================
 * $Author: xuan $
 * $Id: gift_card.php 17217 2012-12-30 00:29:08Z liubo $
 */
/* zhao */
define ( 'IN_ECS', true );
...
if ($_REQUEST ['act'] == 'replenish') {
	...
	if (empty ( $_REQUEST ['goods_id'] )) {
		$link [] = array (
				'text' => $_LANG ['go_back'],
				'href' => 'gift_card.php?act=list' 
		);
		sys_msg ( $_LANG ['replenish_no_goods_id'], 1, $link );
	} else {
		$goods_name = $db->GetOne ( "SELECT goods_name From " . $ecs->table ( 'goods' ) . " WHERE goods_id='" . $_REQUEST ['goods_id'] . "' AND is_real = 0 AND extension_code='gift_card' " );
		if (empty ( $goods_name )) {
			$link [] = array (
					'text' => $_LANG ['go_back'],
					'href' => 'gift_card.php?act=list' 
	...
	$smarty->assign ( 'action_link', array (
			'text' => $_LANG ['go_list'],
			'href' => 'gift_card.php?act=card&goods_id=' . $card ['goods_id'] 
	) );
	$smarty->display ( 'replenish_gift_info.htm' );
}
...
elseif ($_REQUEST ['act'] == 'edit_replenish') {
	...
	admin_priv ( 'giftcard' );
	/* 获取卡片信息 */
	$sql = "SELECT T1.card_id, T1.goods_id, T2.goods_name,T1.card_sn, T1.card_password, T1.patch_id, T1.end_date, T1.crc32 FROM " . $ecs->table ( 'gift_card' ) . " AS T1, " . $ecs->table ( 'goods' ) . " AS T2 " . "WHERE T1.goods_id = T2.goods_id AND T1.card_id = '$_REQUEST[card_id]'";
	$card = $db->GetRow ( $sql );
...
elseif ($_REQUEST ['act'] == 'action') {
	...
	if ($_POST ['card_sn'] != $_POST ['old_card_sn']) {
		$sql = "SELECT count(*) FROM " . $ecs->table ( 'gift_card' ) . " WHERE goods_id='" . $_POST ['goods_id'] . "' AND card_sn='$coded_card_sn'";
		
		if ($db->GetOne ( $sql ) > 0) {
			$link [] = array (
					'text' => $_LANG ['go_back'],
					'href' => 'gift_card.php?act=replenish&goods_id=' . $_POST ['goods_id'] 
			);
	...
	if (empty ( $_POST ['old_card_sn'] )) {
		...
		$add_date = gmtime ();
		$sql = "INSERT INTO " . $ecs->table ( 'gift_card' ) . " (goods_id, card_sn, card_password, end_date, add_date, crc32) " . "VALUES ('$_POST[goods_id]', '$coded_card_sn', '$coded_card_password', '$end_date', '$add_date', '" . crc32 ( AUTH_KEY ) . "')";
		$db->query ( $sql );
		...
		$link [] = array (
				'text' => $_LANG ['go_list'],
				'href' => 'gift_card.php?act=card&goods_id=' . $_POST ['goods_id'] 
		);
		$link [] = array (
		...
		/* 更新数据 */
		$end_date = strtotime ( $_POST ['end_dateYear'] . "-" . $_POST ['end_dateMonth'] . "-" . $_POST ['end_dateDay'] );
		$sql = "UPDATE " . $ecs->table ( 'gift_card' ) . " SET  end_date='$end_date' " . "WHERE card_id='$_POST[card_id]'"; // card_sn='$coded_card_sn',
...
elseif ($_REQUEST ['act'] == 'card') {
	/* 检查权限 */
	admin_priv ( 'giftcard' ); /* zhao */

    /* 验证goods_id是否合法 */
    /* if (empty($_REQUEST['goods_id']))
    {
        $link[] = array('text'=>$_LANG['go_back'],'href'=>'gift_card.php?act=list');
        sys_msg($_LANG['replenish_no_goods_id'], 1, $link);
    }
    else
    {
	...
	$smarty->assign ( 'action_link', array (
			...
			'href' => 'gift_card.php?act=replenish&goods_id=' . $_REQUEST ['goods_id'] 
	) );
	$smarty->assign ( 'goods_id', $_REQUEST ['goods_id'] );
	...
	assign_query_info ();
	$smarty->display ( 'replenish_gift_list.htm' );
}
...
elseif ($_REQUEST ['act'] == 'query_card') {
	if ($_REQUEST ['exportf'] == 1) {
		// 导出到文本;
		$file_name = "gift_card_". date ( "Ymd" ) . ".csv";// . trim ( $_REQUEST ['keyword'] ) . "_" 
...
elseif ($_REQUEST ['act'] == 'batch_drop_card') {
	...
	$sql = "DELETE FROM " . $ecs->table ( 'gift_card' ) . " WHERE card_id " . db_create_in ( implode ( ',', $_POST ['checkboxes'] ) );
	if ($db->query ( $sql )) {
		/* 商品数量减$num */
		update_goods_number ( intval ( $_REQUEST ['goods_id'] ) );
		$link [] = array (
				'text' => $_LANG ['go_list'],
				'href' => 'gift_card.php?act=card&goods_id=' . $_REQUEST ['goods_id'] 
		);
		sys_msg ( $_LANG ['action_success'], 0, $link );
	}
}
...
elseif ($_REQUEST ['act'] == 'batch_card_add') {
	...
	$smarty->assign ( 'action_link', array (
			'text' => $_LANG ['gift_card_list'],
			'href' => 'goods.php?act=list&extension_code=gift_card' 
	) );
	$smarty->assign ( 'goods_id', $_REQUEST ['goods_id'] );
	$smarty->display ( 'batch_card_gift_info.htm' );
} 
...
elseif ($_REQUEST ['act'] == 'batch_confirm') {
	...
	$attr_value = $db->getAll ( "select attr_id,attr_value from ecs_goods_attr where goods_id=" . $goods_id ); // ."
	                                                                                                           // ".$_LANG['rule_attr_id'],
	$attrArr = array ();
	while ( list ( $key, $val ) = each ( $attr_value ) ) {
		// 217卡片规则、218卡密长度、220有效期、221余额
		$attrID = $val ['attr_id'];
	...
	$smarty->assign ( 'action_link', array (
			...
			'href' => 'gift_card.php?act=batch_card_add&goods_id=' . $_REQUEST ['goods_id'] 
	) );
	$smarty->assign ( 'list', $rec );
	$smarty->display ( 'batch_gift_card_confirm.htm' );
} /* 批量上传处理 */
...
elseif ($_REQUEST ['act'] == 'merge_confirm') {
	...
	$cardList = $db->getAll ( "select card_id,card_sn,money,leave_money, end_date from ecs_gift_card where card_id in ($cardIDStr)" );
	 ...
	 * $smarty->assign ( 'action_link', array ( 'text' => $_LANG
	 		* ['batch_card_add'], 'href' =>
	 		* 'gift_card.php?act=batch_card_add&goods_id=' . $_REQUEST ['goods_id'] )
	 		* );
	*/
	$smarty->assign ( 'list', $rec );
	$smarty->display ( 'batch_merge_gift_card_confirm.htm' );
} /* 批量上传处理 */

elseif ($_REQUEST ['act'] == 'delay_gift_card') {
	/* 检查提交变量是否有效 */
	if (! $_POST ['checkboxes']) {
		sys_msg ( $_LANG ['not_set_patchnum'], 1 ); // 修改文本
	}
	...
	*/
	$smarty->assign ( 'list', $rec );
	$smarty->display ( 'batch_delay_gift_card_confirm.htm' );
} /* 批量上传处理 */

elseif ($_REQUEST ['act'] == 'down_gift_card') {
	/* 检查提交变量是否有效 */
	if (! $_POST ['checkboxes']) {
		sys_msg ( $_LANG ['not_set_patchnum'], 1 ); // 修改文本
	}
	// 导出到文本;
	$file_name = "gift_card_" . trim ( $_REQUEST ['keyword'] ) . "_" . date ( "Ymd" ) . ".csv";
...
elseif ($_REQUEST ['act'] == 'merge_card') {
	...
	// echo "select card_id,leave_money from ecs_gift_card where card_id in
	// ($cardIDStr)";
	$cardList = $db->getAll ( "select card_id,leave_money from ecs_gift_card where card_id in ($cardIDStr)" );
	$count_leave_money = 0;
	while ( list ( $key, $val ) = each ( $cardList ) ) {
	...
	$sql = "UPDATE ecs_gift_card SET leave_money=leave_money+" . $count_leave_money . " ,update_time='" . date ( "Y-m-d H:i:s" ) . "' WHERE 1 and " . $where;
	
	$GLOBALS ['db']->query ( $sql );
	/* 更新商品库存 */
	$link [] = array (
			'text' => $_LANG ['card'],
			'href' => 'gift_card.php?act=gift_card&goods_id=' . $_POST ['goods_id'] 
	);
...
elseif ($_REQUEST ['act'] == 'delay_card') {
	...
	$updateCArr ['update_time'] = date ( "Y-m-d H:i:s" );
	$sql = "UPDATE ecs_gift_card SET end_date=" . $updateCArr ['end_date'] . " ,update_time='" . date ( "Y-m-d H:i:s" ) . "' WHERE 1 and card_id in (".$cardIDStr.")";
...
elseif ($_REQUEST ['act'] == 'batch_insert') {
	...
	foreach ( $_POST ['checked'] as $key ) {
		...
		$db->AutoExecute ( $ecs->table ( 'gift_card' ), $rec, 'INSERT' );
		$i ++;
	}
//	print_r($_REQUEST);
	/* 更新商品库存 */
...
elseif ($_REQUEST ['act'] == 'change') {
	...
	$smarty->assign ( 'ur_here', $_LANG ['gift_card_change'] );
	
	assign_query_info ();
	$smarty->display ( 'gift_card_change.htm' );
}
...
elseif ($_REQUEST ['act'] == 'submit_change') {
	...
	if (isset ( $_POST ['old_string'] ) && isset ( $_POST ['new_string'] )) {
		...
		$sql = "SELECT card_id, card_sn, card_password FROM " . $ecs->table ( 'gift_card' ) . " WHERE crc32 = '$old_crc32'";
		$res = $db->query ( $sql );
		while ( $row = $db->fetchRow ( $res ) ) {
			...
			$row ['crc32'] = $new_crc32;
			$db->autoExecute ( $ecs->table ( 'gift_card' ), $row, 'UPDATE', 'card_id = ' . $row ['card_id'] );
		}
...
elseif ($_REQUEST ['act'] == 'toggle_sold') {
	check_authz_json ( 'giftcard' );
	
	$id = intval ( $_POST ['id'] );
	$val = intval ( $_POST ['val'] );
	
	$sql = "UPDATE " . $ecs->table ( 'gift_card' ) . " SET is_saled= '$val' WHERE card_id='$id'";
	
	if ($db->query ( $sql, 'SILENT' )) {
		/* 修改商品库存 */
		$sql = "SELECT goods_id FROM " . $ecs->table ( 'gift_card' ) . " WHERE card_id = '$id' LIMIT 1";
...
elseif ($_REQUEST ['act'] == 'remove_card') {
	...
	$id = intval ( $_GET ['id'] );
	echo "\nid:" . 'SELECT card_sn, goods_id FROM ' . $ecs->table ( 'gift_card' ) . " WHERE card_id = '$id'";
	$row = $db->GetRow ( 'SELECT card_sn, goods_id FROM ' . $ecs->table ( 'gift_card' ) . " WHERE card_id = '$id'" );
	
	$sql = 'DELETE FROM ' . $ecs->table ( 'gift_card' ) . " WHERE card_id = '$id'";
	if ($db->query ( $sql, 'SILENT' )) {
		/* 修改商品数量 */
		update_goods_number ( $row ['goods_id'] );
		
		echo $url = 'gift_card.php?act=query_card&' . str_replace ( 'act=remove', '', $_SERVER ['QUERY_STRING'] );
...
elseif ($_REQUEST ['act'] == 'start_change') {
	...
	$sql = "SELECT crc32, count(*) AS cnt FROM " . $ecs->table ( 'gift_card' ) . " GROUP BY crc32";
...
elseif ($_REQUEST ['act'] == 'on_change') {
	...
	$sql = "SELECT card_id, card_sn, card_password " . " FROM " . $ecs->table ( 'gift_card' ) . " WHERE crc32 = '$old_crc32' LIMIT $each_num";
	$res = $db->query ( $sql );
	...
	while ( $row = $db->fetchRow ( $res ) ) {
		...
		if (! $db->autoExecute ( $ecs->table ( 'gift_card' ), $row, 'UPDATE', 'card_id = ' . $row ['card_id'] )) {
	...
	// 查询是否还有未更新的
	$sql = "SELECT COUNT(*) FROM " . $ecs->table ( 'gift_card' ) . " WHERE crc32 = '$old_crc32' ";
	...
	if ($left_num > 0) {
		...
		$sql = "SELECT crc32, count(*) AS cnt FROM " . $GLOBALS ['ecs']->table ( 'gift_card' ) . " GROUP BY crc32";
		$res = $GLOBALS ['db']->query ( $sql );
		while ( $row = $db->fetchRow ( $res ) ) {
...
elseif ($_REQUEST ['act'] == 'del_gift_card')
{
    $card_id = $_POST['card_id'];
    if($card_id>0)
    {
        $sql = "UPDATE " . $GLOBALS ['ecs']->table ( 'gift_card' ) . " SET card_status=3 WHERE 1 $where AND card_id=$card_id AND card_status<>3";
        $res = $GLOBALS ['db']->query ( $sql );
        if($res)
        {
            make_json_result("该卡已作废");
        }
        else
        {
            make_json_error("该卡无法作废");
        }
    }
    else
    {
        make_json_error("礼品卡ID异常");
    }
}
/**
 * 获取要导出的卡片内容
...
function get_export_list($idArr) {
	...
	$sql = "SELECT card_id, goods_id, card_sn, card_password, end_date, use_status, card_status, order_sn,patch_id " . " FROM " . $GLOBALS ['ecs']->table ( 'gift_card' ) . " WHERE 1 " . $where . " ORDER BY card_id asc ";
	...
	$sql = "UPDATE " . $GLOBALS ['ecs']->table ( 'gift_card' ) . " SET use_status=2 WHERE 1 $where AND use_status!=3";
	$GLOBALS ['db']->query ( $sql );
	
	//更新库存
	$sql_g = "SELECT distinct goods_id " . " FROM " . $GLOBALS ['ecs']->table ( 'gift_card' ) . " WHERE 1 " . $where ;
	$goodsArr = $GLOBALS ['db']->getAll ( $sql_g );
	while (list($key,$goods_id) = each($goodsArr)) {
		update_goods_number ( intval ( $goods_id['goods_id']) );
	}
...
function get_replenish_list($exportf = 0) {
	...
	$sql = "SELECT COUNT(*) FROM " . $GLOBALS ['ecs']->table ( 'gift_card' ) . " WHERE 1 $where";
	...
	/* 查询 */
	$sql = "SELECT card_id, goods_id, card_sn, card_password,money,leave_money, end_date, use_status, card_status, order_sn,patch_id " . " FROM " . $GLOBALS ['ecs']->table ( 'gift_card' ) . " WHERE 1 " . $where . " ORDER BY $filter[sort_by] $filter[sort_order] " . " LIMIT $start, $filter[page_size]";
	$all = $GLOBALS ['db']->getAll ( $sql );
...
function update_goods_number($goods_id) {
	$sql = "SELECT COUNT(*) FROM " . $GLOBALS ['ecs']->table ( 'gift_card' ) . " WHERE goods_id = '$goods_id' AND use_status = 1";
	$goods_number = $GLOBALS ['db']->getOne ( $sql );
	
	$sql = "UPDATE " . $GLOBALS ['ecs']->table ( 'goods' ) . " SET goods_number = '$goods_number' WHERE goods_id = '$goods_id' AND extension_code='gift_card'";
...
function update_card_status($where, $updateArr) {
	...
	echo $sql = "UPDATE ecs_gift_card SET " . $upStr . " WHERE 1 and " . $where;
 ...
 * @return 礼品卡卡号和密码
 */
function get_gift_card($ruleArr, $patchID, $xid) {
	$ruleStr = $ruleArr ['217'];
	$passLen = $ruleArr ['218'];
	$money = $ruleArr ['221'];
	$res = array ();
	// WJ+YY+MM+DD+PP+dddd+xxxx
	$info = explode ( "+", $ruleStr );
	$res ['sn'] = "WJ" . date ( "Ymd" ) . sprintf ( "%02s", $patchID ) . sprintf ( "%0" . strlen ( $info [5] ) . "d", $money ) . sprintf ( "%0" . strlen ( $info [6] ) . "s", $xid );
