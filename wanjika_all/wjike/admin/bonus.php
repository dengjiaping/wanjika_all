...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $except_goodsid   = !empty($_POST['except_goodsid'])  ? trim($_POST['except_goodsid'])    : '';
    $except_goodsid = str_replace('，',',',$except_goodsid);
    $admin_show   = !empty($_POST['admin_show']) ? trim($_POST['admin_show']) : '';
...
//    }
    ...
    $send_enddate   = local_strtotime($_POST['send_end_date']) + 24 * 3600-1;
    $use_startdate  = local_strtotime($_POST['use_start_date']);
    $use_enddate    = local_strtotime($_POST['use_end_date']) + 24 * 3600-1;
    $is_limit  = !empty($_POST['is_limit']) ? 1 : 0;
    $use_datetime  = !empty($_POST['use_datetime']) ? 1 : 0;
    $use_effective_date  = !empty($_POST['use_effective_date']) ? intval($_POST['use_effective_date']) : 0;

    /* 插入数据库。 */
    $sql = "INSERT INTO ".$ecs->table('bonus_type')." (type_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,min_amount,note,min_goods_amount,is_limit,use_datetime,use_effective_date,admin_show,cannot_use_ids)
    VALUES ('$type_name',
            '$_POST[type_money]',
            '$send_startdate',
            '$send_enddate',
            ...
            '$_POST[send_type]',
            '$min_amount',
            '$note','" . floatval($_POST['min_goods_amount']) . "',
            '$is_limit',
            '$use_datetime',
            '$use_effective_date',
            '$admin_show',
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $use_goods_list = get_bonus_use_goods($type_id);
    $except_use_goods_list = get_except_use_goods($type_id);

    $pay_ids = get_bonus_pay_ids($bonus_arr);
    /* 取得优惠范围 */
    $act_range_ext = array();
    if ($bonus_arr['act_range'] != 0 && $bonus_arr['act_range'] != 3 && !empty($bonus_arr['act_range_ext']))
    {
        if ($bonus_arr['act_range'] == 1)
        {
            $sql = "SELECT cat_id AS id, cat_name AS name FROM " . $ecs->table('category') .
                " WHERE cat_id " . db_create_in($bonus_arr['act_range_ext']);
        }
        elseif ($bonus_arr['act_range'] == 2)
        {
    ...
    $smarty->assign('use_goods_list',  $use_goods_list);
    $except_goodsid = '';
    foreach($except_use_goods_list as $v){
        $except_goodsid .= ($v['goods_id'].',');
    }
    $except_goodsid = substr($except_goodsid, 0, -1);
    $smarty->assign('except_goodsid',  $except_goodsid);
    $smarty->assign('payment_list', $pay_ids);
...
if ($_REQUEST['act'] == 'update')
{
    ...
    //应用范围
    $use_goods_ids = array();
    if (!empty($_POST['use_goods_target_select']))
    {
        foreach ($_POST['use_goods_target_select'] as $val)
        {
            $use_goods_ids[] = intval($val);
        }
    }
    /* 检查优惠范围扩展信息 */
    if ($act_range >0 && $act_range < 3 && !isset($_POST['act_range_ext']))
    {
        sys_msg('请设置优惠范围');
    }
    $use_goods_ids = array_unique(array_filter($use_goods_ids));
    $use_goods_ids_str = '';
    if (!empty($use_goods_ids))
    {
        $use_goods_ids_str = implode(',', $use_goods_ids);
    }

    //优惠券适用支付方式
    $pay_ids = array();
    if (!empty($_POST['payments_list']))
    {
        foreach ($_POST['payments_list'] as $val)
        {
            $pay_ids[] = intval($val);
        }
    }
    ...
    if(!empty($_POST['act_range_ext']) && $act_range != 0 && $act_range != 3)
    {
        $act_range_ext = $act_range == 3 ? '' : join(',', $_POST['act_range_ext']);
    }
    // 1:分类 2:品牌 3:全部商品
    ...
    if ($act_range == 1)
    {
        foreach($_POST['act_range_ext'] AS $key=>$value)
        {
            if($key == 0)
            {
                $where .= ' AND g.cat_id ' . db_create_in(array_unique(array_merge(array($value), array_keys(cat_list($value, 0, false)))));
            }
            else
            {
                $where = str_replace(array(")"),"",$where);
                $b =db_create_in(array_unique(array_merge(array($value), array_keys(cat_list($value, 0, false)))));
                $b = str_replace(array("IN","("," "),"",$b);
                $where .= ',' . $b . " " ;
            }
        }
    }
    elseif($act_range == 2)
    {
        foreach($_POST['act_range_ext'] AS $key=>$value)
        {
            if($key == 0)
            {
                $where .= ' AND brand_id ' . db_create_in(array_unique(array_merge(array($value), array_keys(cat_list($value, 0, false)))));
            }
            else
            {
                $where = str_replace(array(")"),"",$where);
    ...
    if($act_range != 0 )
    {
        ...
        $use_goods_ids_str = $act_range == 3 ? '' : join(',', $result);
    }
...
if ($_REQUEST['act'] == 'send_by_user')
{
    ...
    foreach ($user_list AS $key => $val)
    {
        ...
        $now = gmtime();
        //限制红包只允许拥有一张
        if($bonus_type['is_limit'] == 1)
        {
            /* 查询是否已经存在该红包 */
            $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('user_bonus') .
                " WHERE bonus_type_id = '$_REQUEST[id]' AND user_id='$val[user_id]' ";
        ...
        if (add_to_maillist($val['user_name'], $val['email'], $tpl['template_subject'], $content, $tpl['is_html']))
        {
            ...
            $sql = "INSERT INTO " . $ecs->table('user_bonus') .
                    ...
                    "VALUES ('$_REQUEST[id]', 0, '$val[user_id]', 0, 0, " .BONUS_MAIL_SUCCEED. ",'$now','$bonus_type[use_start_date]','$bonus_type[use_end_date]')";
            $db->query($sql);
        ...
        else
        {
            ...
            $sql = "INSERT INTO " . $ecs->table('user_bonus') .
                    "(bonus_type_id, bonus_sn, user_id, used_time, order_id, emailed,binding_time,use_start_datetime,use_end_datetime) " .
                    "VALUES ('$_REQUEST[id]', 0, '$val[user_id]', 0, 0, " .BONUS_MAIL_FAIL. ",'$now','$bonus_type[use_start_date]','$bonus_type[use_end_date]')";
            $db->query($sql);
            $new_bonus_id = $db->insert_id();
        }
        if($bonus_type['use_datetime'] == 1)
        {
            //按起止日期计算
            $use_effective_date = $now+$bonus_type['use_effective_date']*24 * 3600;
    ...
    sys_msg(sprintf($_LANG['sendbonus_count'].$user_name, $count), 0, $link,false);
}
...
if ($_REQUEST['act'] == 'send_by_print')
{
    ...
    for ($i = 0, $j = 0; $i < $bonus_sum; $i++)
    {
        ...
        $db->query("INSERT INTO ".$ecs->table('user_bonus')." (bonus_type_id, bonus_sn,binding_time) VALUES('$bonus_typeid', '$bonus_sn','$now')");
...
//-- 导出线下发放的信息
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'download_excel')
{
    @set_time_limit(0);

    /* 获得此线下红包类型的ID */
    $tid = empty($_REQUEST['bonus_type']) ? 0 : intval($_REQUEST['bonus_type']);
    $type_name = $db->getOne("SELECT type_name FROM ".$ecs->table('bonus_type')." WHERE type_id = '$tid'");
    ...
    if (EC_CHARSET != 'gbk')
    {
        echo ecs_iconv('UTF8', 'GB2312', $_LANG['bonus_excel_file']) . "\t\n";
        /* 红包序列号, 红包金额, 类型名称(红包名称), 使用结束日期 */
        echo ecs_iconv('UTF8', 'GB2312', $_LANG['bonus_id']) ."\t";
        echo ecs_iconv('UTF8', 'GB2312', $_LANG['bonus_sn']) ."\t";
        echo ecs_iconv('UTF8', 'GB2312', $_LANG['bonus_type']) ."\t";
        echo ecs_iconv('UTF8', 'GB2312', $_LANG['order_id']) ."\t";
        echo ecs_iconv('UTF8', 'GB2312', $_LANG['user_id']) ."\t";
        echo ecs_iconv('UTF8', 'GB2312', $_LANG['used_time']) ."\t";
        echo ecs_iconv('UTF8', 'GB2312', "绑定时间") ."\t\n";
    }
    else
    {
        echo $_LANG['bonus_excel_file'] . "\t\n";
        ...
        echo $_LANG['bonus_id'] ."\t";
        echo $_LANG['bonus_sn'] ."\t";
        echo $_LANG['bonus_type'] ."\t";
        echo $_LANG['order_id'] ."\t";
        echo $_LANG['user_id'] ."\t";
        echo $_LANG['used_time'] ."\t";
        echo "绑定时间" ."\t\n";
    }
    $sql = "SELECT ub.*, u.user_name, u.email, o.order_sn, bt.type_name ".
        ...
        " LEFT JOIN " .$GLOBALS['ecs']->table('order_info'). " AS o ON o.order_id=ub.order_id WHERE bt.type_id = '$tid' ";
    $row = $GLOBALS['db']->getAll($sql);

    foreach ($row AS $key => $val)
    {
        $row[$key]['used_time'] = $val['used_time'] == 0 ?
            $GLOBALS['_LANG']['no_use'] : local_date($GLOBALS['_CFG']['time_format'], $val['used_time']);
        $row[$key]['binding_time'] = $val['binding_time'] == 0 ?
            '未绑定' : local_date($GLOBALS['_CFG']['time_format'], $val['binding_time']);
    ...
    foreach($row AS $val)
    {
        ...
        echo $val['order_sn'] . "\t";
        echo $val['user_name'] . "\t";
        echo ecs_iconv('UTF8', 'GB2312', $val['used_time']) . "\t";
        echo $val['binding_time'] . "\t";
        echo "\t\n";
    }
}
/*------------------------------------------------------ */
...
if ($_REQUEST['act'] == 'get_goods_list')
{
    ...
    $arr = get_goods_list($filters,true);
    $opt = array();
...
if ($_REQUEST['act'] == 'search_users')
{
    ...
    $starttime = json_str_iconv(trim($_GET['starttime']));
    $endtime = json_str_iconv(trim($_GET['endtime']));
    ...
    if(!empty($keywords)){
        $where .= " and user_name LIKE '%" . mysql_like_quote($keywords) . "%' OR user_id LIKE '%" . mysql_like_quote($keywords) . "%'";
    }
...
if ($_REQUEST['act'] == 'update_use_goodsid')
{
    //上传订单文件
...
//        }
        ...
        */
        ...
        for($j=1;$j<=$highestRow;$j++)
        {
            ...
            $use_goodsid = $use_goodsid.$a.',';

        }
        $use_goodsid = substr($use_goodsid, 0, -1);
        $type_id = $_REQUEST["type_id"];

        $sql = 'UPDATE ' . $GLOBALS['ecs']->table('bonus_type') . " SET use_goods_ids='$use_goodsid' WHERE type_id = $type_id";
...
function get_bonus_use_goods($type_id)
{
    $sql = "SELECT type_id, use_goods_ids" .
        " FROM " . $GLOBALS['ecs']->table('bonus_type') .
        " WHERE type_id = '$type_id'";

    $ret = $GLOBALS['db']->getRow($sql);

    $use_goods_infos = array();
    if (!empty($ret['use_goods_ids']))
    {
        $sql = "SELECT goods_id, goods_name FROM " . $GLOBALS['ecs']->table('goods') .
            " WHERE goods_id in (" . $ret['use_goods_ids'] . ")";
        ...
        if (!empty($goods))
        {
            ...
            {
                $use_goods_infos[] = array('goods_id' => $info['goods_id'], 'goods_name' => $info['goods_name']);
            }
        }
    }


    return $use_goods_infos;
}

function get_except_use_goods($type_id)
{
    $sql = "SELECT type_id, cannot_use_ids" .
        " FROM " . $GLOBALS['ecs']->table('bonus_type') .
        " WHERE type_id = '$type_id'";

    $ret = $GLOBALS['db']->getRow($sql);

    $except_use_goods_infos = array();
    if (!empty($ret['cannot_use_ids']))
    {
        $sql = "SELECT goods_id, goods_name FROM " . $GLOBALS['ecs']->table('goods') .
            " WHERE goods_id in (" . $ret['cannot_use_ids'] . ")";
        ...
        if (!empty($goods))
        {
            foreach ($goods as $info)
            {
                $except_use_goods_infos[] = array('goods_id' => $info['goods_id'], 'goods_name' => $info['goods_name']);
            }
        }
    }

    return $except_use_goods_infos;
}

function get_bonus_pay_ids($bonus_arr)
{
    $pay_ids = array();
    if (!empty($bonus_arr['pay_ids']))
    {
        $pay_ids = explode(',', $bonus_arr['pay_ids']);
    }
    ...
    if (!empty($payments))
    {
        ...
        {
            $arr = array('pay_id' => $info['pay_id'], 'pay_name' => $info['pay_name']);
            if (in_array($info['pay_id'], $pay_ids))
            {
                $arr['selected'] = 1;
            }
