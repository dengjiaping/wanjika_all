...
if (empty($brand_id))
{
    show_message("错误的地址","去首页","index.php","warning");
    /* 缓存编号 */
...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price','salesnum', 'last_update'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
...
if(strpos($_SERVER["REQUEST_URI"],'.html') !== false){
    $order = 'DESC';
    if(strpos($_SERVER["REQUEST_URI"],'ASC') !== false){
        $order = 'ASC';
    }
    $sort = 'last_update';
    if(strpos($_SERVER["REQUEST_URI"],'salesnum') !== false){
        $sort = 'salesnum';
    }
    if(strpos($_SERVER["REQUEST_URI"],'shop_price') !== false){
        $sort = 'shop_price';
    }
}
...
if (!$smarty->is_cached('brand.dwt', $cache_id))
{
    ...
    $smarty->assign('page_title',     htmlspecialchars($brand_info['brand_title']));   // 页面标题
    $smarty->assign('ur_here',        $position['ur_here']); // 当前位置
    ...
    {
        show_message("该品牌下无商品","去首页","index.php","warning");
    }
...
function brand_get_goods($brand_id, $cate, $size, $page, $sort, $order)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        foreach($country_array AS $key=>$value)
        {
            if($row['overseas_logo']  == $value['country_code'])
            {
                $arr[$row]['overseas_logo_name'] = $value['country_name'];
            }
        }
