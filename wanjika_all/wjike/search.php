...
if ($_REQUEST['act'] == 'advanced_search')
{
    ...
    $smarty->assign('search_keywords',    $_REQUEST['keywords']);  //搜索内容
...
else
{
    ...
    if (!empty($_REQUEST['keywords']))
    {
        ...
        if($_REQUEST['keywords'] == "亚洲馆")
        {
            $request_key = "韩国 日本 泰国 台湾";
        }
        else if($_REQUEST['keywords'] == "欧洲馆")
        {
            $request_key = "丹麦 德国 法国 荷兰 瑞典 塞尔维亚 意大利 英国";
        }
        else if($_REQUEST['keywords'] == "北美馆")
        {
            $request_key = "加拿大 美国";
        }
        else if($_REQUEST['keywords'] == "澳新馆")
        {
            $request_key = "澳大利亚 新西兰";
        }
        else
        {
            $request_key = $_REQUEST['keywords'];
        }
        $arr = array();
        if (stristr($request_key, ' AND ') !== false)
        {
            /* 检查关键字中是否有AND，如果存在就是并 */
            $arr        = explode('AND', $request_key);
            $operator   = " AND ";
        }
        elseif (stristr($request_key, ' OR ') !== false)
        {
            /* 检查关键字中是否有OR，如果存在就是或 */
            $arr        = explode('OR', $request_key);
            $operator   = " OR ";
        }
        elseif (stristr($request_key, ' + ') !== false)
        {
            /* 检查关键字中是否有加号，如果存在就是或 */
            $arr        = explode('+', $request_key);
            $operator   = " OR ";
        }
        else
        {
        ...
        foreach ($arr AS $key => $val)
        {
            ...
            $keywords  .= "(goods_name LIKE '%$val%' OR goods_sn LIKE '%$val%' OR keywords LIKE '%$val%' OR gc.country_name LIKE '%$val%' $sc_dsad)";
    ...
    $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
                ...
                "g.promote_price, g.promote_start_date, g.promote_end_date, g.goods_thumb, g.original_img, g.goods_brief, g.goods_type,gs.is_overseas,g.is_immediately,g.overseas_logo ".
