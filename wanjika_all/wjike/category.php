...
if (!$smarty->is_cached('category.dwt', 1))
{
    /* 如果页面没有被缓存则重新获取页面的内容 */
    ...
    $ur_here_id=$position['ur_here_id'];
    $temp_arrt = get_parent_cats($cat_id);
    $temp_arrtt = end($temp_arrt);
    $temp_cat_id =$temp_arrtt['cat_id'];
    $gethotgoods = get_recommend_goods('hot');
    $ur_here_url = $temp_cat_id.".html";
    if(is_local()){
        $ur_here_url = "category.php?id=".$temp_cat_id;
    }
    foreach($gethotgoods as $val)
    {
        $getcat_id=$val['cat_id'];
        $sq='SELECT parent_id ' .
            'FROM ' . $GLOBALS['ecs']->table('category') .
            "WHERE cat_id='$getcat_id'";
        $row= $GLOBALS['db']->getOne($sq);
        if($temp_cat_id==$row || $temp_cat_id==$getcat_id)
        {
            $goods_hot[]=$val;
        }
    }
    ...
    $smarty->assign('ur_here_name',    $position['ur_here_name']);  // 当前分类
    $smarty->assign('ur_here_url',    $ur_here_url);  //分类原始点
    $smarty->assign('goods_hot',       $goods_hot);     // 热点文章
    $smarty->assign('categoriess',       $categories_tree[$ur_here_id]); // 分类树
...
//    $smarty->assign('venues',          get_all_venue()); // 场馆类
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                ...
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.original_img,gs.is_overseas,g.is_immediately,g.overseas_logo ' .
            'FROM (' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ) LEFT JOIN ". $GLOBALS['ecs']->table('goods_supplier') ." AS gs ON g.supplier_id=gs.type_id " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['is_immediately']      = $row['is_immediately'];
        ...
        $arr[$row['goods_id']]['url']              = 'http://item.wjike.com/'.$row['goods_id'].'.html';
        if(is_local()){
