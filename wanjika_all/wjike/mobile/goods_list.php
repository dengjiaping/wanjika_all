...
if ($num > 0)
{
	...
	foreach ($sort_array as $goods_key)
	{
		if (($i > ($page_num * ($page - 1 ))) && ($i <= ($page_num * $page)))
		{
			...
			$goods_data[] = array('i' => $i , 'price' => encode_output($price) , 'id' => $goods[$goods_key]['id'] , 'name' => encode_output($goods[$goods_key]['name']), 'thumb' => $goods[$goods_key]['thumb'], 'is_overseas' => $goods[$goods_key]['is_overseas']);//16:43 2013-07-16
		}
		$i++;
	}
...
function wap_get_recommend_goods($type = '', $order_rule = '')
{
	...
	if (empty($type_goods[$type]))
	{
		...
		$sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price,gs.is_overseas, ' .
...
function wap_get_promote_goods($order_rule = '')
{
	...
	$sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
				"IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
				"promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, goods_img,gs.is_overseas, b.brand_name, " .
