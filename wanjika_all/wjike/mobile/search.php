...
$_REQUEST['user_id'] = !empty($_REQUEST['user_id']) ? intval($_REQUEST['user_id']) : 0;
/*------------------------------------------------------ */
//-- 高级搜索
/*------------------------------------------------------ */
...
else
{
    ...
    {
        $_REQUEST['sort'] = 'last_update';
    }
    else
    {
        $_REQUEST['sort'] = 'shop_price';
        if($_REQUEST['order_price'] == 2)
        {
            $_REQUEST['order'] = 'ASC';
        }
        $smarty->assign('order_price',$_REQUEST['order_price']);
    }
    if (!empty($_REQUEST['last']))
    {
        $_REQUEST['sort'] = 'last_update';
    }
    if (!empty($_REQUEST['salesnum']))
    {
        $_REQUEST['sort'] = 'salesnum';
    }
    if (!empty($_REQUEST['new_goods']))
    {
        $_REQUEST['sort'] = 'goods_id';
    }
	$action = '';
	...
	if (!empty($_REQUEST['keywords']))
	{
		...
		$keywords .= ')';
        //增加搜索历史记录
        $db->autoReplace($ecs->table('keywords_history'), array('date' => local_date('Y-m-d'),
            'keyword' => $_REQUEST['keywords'], 'user_id' => $_REQUEST['user_id'], 'count' => 1), array('count' => 1));
    ...
    else
    {
        ...
        {
            $sql_history = "SELECT keyword FROM " . $ecs->table("keywords_history") . " WHERE user_id=$_REQUEST[user_id] LIMIT 10";
            $row_history = $db->getAll($sql_history);
        }
        $smarty->assign('history', $row_history);
        $smarty->assign('s_empty', true);
        $smarty->assign('footer', get_footer());
        $smarty->display('search.html');exit;
    }
	...
	/* 查询商品 */
	$sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.is_new, g.is_best, g.is_hot,g.overseas_logo,gs.is_overseas, g.shop_price AS org_price, ".
				"IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
				"g.promote_price, g.promote_start_date, g.promote_end_date, g.goods_thumb, g.goods_img, g.goods_brief, g.goods_type,gs.is_overseas ".
			"FROM " .$ecs->table('goods'). " AS g ".
			"LEFT JOIN " . $GLOBALS['ecs']->table('goods_supplier') . " AS gs ".
					"ON g.supplier_id=gs.type_id ".
	...
	while ($row = $db->FetchRow($res))
	{
        ...
        $arr[$row['goods_id']]['is_overseas']         = $row['is_overseas'];
        $arr[$row['goods_id']]['overseas_logo']         = $row['overseas_logo'];
        $arr[$row['goods_id']]['overseas_logo_name']='';
        $country_array = country_list();
        foreach($country_array AS $key=>$value)
        {
            if($arr[$row['goods_id']]['overseas_logo'] == $value['country_code'])
            {
                $arr[$row['goods_id']]['overseas_logo_name'] = $value['country_name'];
            }
        }
		...
		$arr[$row['goods_id']]['price']	= price_format($row['shop_price']);
        $arr[$row['goods_id']]['goods_discount']        = number_format($row['shop_price']/$row['market_price']*10, 1, '.', '');
        $arr[$row['goods_id']]['market_price'] = price_format($row['market_price']);
        $arr[$row['goods_id']]['promote_pri'] = $promote_price;
		$arr[$row['goods_id']]['promote_price'] = ($promote_price > 0) ? price_format($promote_price) : '';
		$arr[$row['goods_id']]['url']		   = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
		$arr[$row['goods_id']]['goods_thumb']	  = $row['goods_thumb'];//16:53 2013-07-16
		$arr[$row['goods_id']]['thumb']	  = $row['goods_thumb'];//16:53 2013-07-16
		$arr[$row['goods_id']]['is_overseas']	  = $row['is_overseas'];
	}
    if($_REQUEST['search_load']==1)
    {
