...
{
//    $back_act ='http://'. $GLOBALS['_SERVER']['HTTP_HOST'].'/mobile/cart.php';
...
elseif($act=='update_group_cart')
{
	...
	else
	{
		...
		foreach($cart_goods['goods_list'] as $rec_goods)
		{
			...
			{
                $result['goods_price'] = $rec_goods['goods_price'];
		...
		$result['number'] = $cart_goods['total']['real_goods_count'];
		$result['discount'] = price_format($discount['discount']);
		$result['subtotal_exchange_integral'] = empty($subtotal_exchange_integral) ? 0 : $subtotal_exchange_integral;
		$result['cart_amount_desc'] = price_format($cart_goods['total']['goods_amount']-$discount['discount']);
...
$cart_goods['total']['goods_price'] = price_format($cart_goods['total']['goods_amount']-$discount['discount']);
