 ...
 * ============================================================================
 * $Author: liubo $
 * $Id: usertest.php 16643 2009-09-08 07:02:13Z liubo $
*/
	...
	else
	{
		if ($user->check_user($user_name, $pwd) > 0)
		{
            ...
            if($weixin_binding == "1"){
                ...
                if($is_weixin){
                    ...
                    if(!$is_binding && !empty($openid)){
                        binding_weixin($openid,$_SESSION['user_id'],$user_name);
			...
			if ($db->getOne($sql) > 0){
				...
				ecs_header("Location:usertest.php\n");
				exit;
			}
		}
		else
...
//	$smarty->display('logintest.html');
//	exit;
    ecs_header("Location: usertest.php?act=login&login_faild=$login_faild&user_name=$user_name\n");
    exit;
}
if ($act == 'weixin_login')
{
...
elseif ($act == 'login')
{
    ...
    if (empty($back_act))
    {
        ...
        {
            $back_act = strpos($GLOBALS['_SERVER']['HTTP_REFERER'], 'usertest.php') ? './index.php' : $GLOBALS['_SERVER']['HTTP_REFERER'];
        }
        else
        {
            $back_act = 'usertest.php';
        }
    }
    if(!$is_binding || $flag == 'true'){
        ...
        $user_name = get_binding_user($openid);
        $smarty->assign('user_name', $user_name);
        $smarty->display('bind_logintest.html');
    }
}
//我的礼品卡
elseif ($act == 'gift_card')
{
    ...
    $pager = get_pager('usertest.php', array('act' => $action), $record_count, $page);
...
//        $smarty->display('logintest.html');
//        exit;
        ecs_header("Location: usertest.php?act=login\n");
        exit;
    }
    ...
    $unavl_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('user_bonus'). " AS u ,".
        ...
        " WHERE u.bonus_type_id = b.type_id AND b.use_end_date < $cur_date AND order_id = 0 AND u.user_id = '" .$user_id. "'");
    ...
    $pager = get_pager('usertest.php', array('act' => $action,'status' => $status), $record_count, $page);
    if($_REQUEST['order_flow']!=1)
    {
...
elseif ($act == 'drop_consignee')
{
    ...
    if($flowconsignee_id==1)
    {
        ...
        {
            ecs_header("Location: usertest.php?act=address_list&address_flow=1&bonus_id=$bonus_id&gift_id=$gift_id\n");
            exit;
        }
        else
        {
    ...
    else
    {
        if (drop_consignee($consignee_id))
        {
            ecs_header("Location: usertest.php?act=address_list\n");
            exit;
        }
        else
        {
...
//		exit;
	...
	if ($record_count > 0)
	{
		...
		$pagebar = get_wap_pager($record_count, $page_num, $page, 'usertest.php?act=order_list&status='.$status, 'page');
...
//		exit;
	...
	require_once(ROOT_PATH . 'languages/' .$_CFG['lang']. '/usertest.php');
	/* 订单商品 */
...
//		exit;
	...
	{
		ecs_header("Location: usertest.php?act=order_list\n");
		exit;
	}
}
...
elseif ($act == 'act_register')
{
    if($_CFG['shop_reg_closed']){
        ...
        if (strlen($username) < 11)
        {
            ...
            ecs_header("Location:./usertest.php?act=register\n");
        }
...
//            {
//                show_regmessage($_LANG['invalid_captcha'], $_LANG['sign_up'], 'usertest.php?act=register', 'error');
//            }
//
//            /* 检查验证码 */
...
//        }
        ...
        {
            show_regmessage('注册失败！', $_LANG['sign_up'], 'usertest.php?act=register', 'error');
        }
    }
}
...
elseif ($act == 'get_vcode')
{
    ...
    $json = new JSON;
    $phone = $_POST['phone'];
    if($_SERVER['HTTP_REFERER'] != 'http://'.$_SERVER['HTTP_HOST'].'/mobile/usertest.php?act=register'){
        $r['errorcode']   = 3;
        $r['msg'] = '短信接口错误';
        die($json->encode($r));
    }
    ...
    $result = $mysms->send($phone,$msg,'sdkapt',1);
...
elseif ($act == 'act_forgetpassword')
{
    ...
    {
        $_SESSION['errormsg'] = '<span style="background-color: #ffa200;padding: 0px 6px;border-radius: 8px;color: #fff;margin-right: 4px;">!</span>输入手机号位数少于11位';
        ecs_header("Location:./usertest.php?act=forget_password\n");
    }
...
//    }
    ...
    if ($user->edit_user(array('username'=> $user_info['user_name'], 'old_password'=>'', 'password'=>$password), empty($code) ? 0 : 1))
    {
        ...
        $db->query($sql);
        $user->logout();
        show_regmessage($_LANG['edit_password_success'], $_LANG['relogin_lnk'], 'usertest.php?act=login');
    }
    else
    {
        show_regmessage($_LANG['edit_password_failure'], $_LANG['sign_up'], 'usertest.php?act=register', 'error');
    }
...
elseif ($act == 'get_forgetvcode')
{
    ...
    if($_SERVER['HTTP_REFERER'] != 'http://'.$_SERVER['HTTP_HOST'].'/mobile/usertest.php?act=forget_password'){
        $r['errorcode']   = 3;
        $r['msg'] = '短信接口错误';
        die($json->encode($r));
    }
...
elseif ($act == 'follow')
{
    ...
    $pager = get_pager('usertest.php', array('act' => $action), $record_count, $page, 8);
    $smarty->assign('pager', $pager);
...
//        exit;
    ...
    {
        ecs_header("Location: usertest.php?act=order_info&id=$order_id\n");
        exit;
    }
            ...
            WeixinJSBridge.invoke(
                    ...
                    function(res){
                        if(res.err_msg == 'get_brand_wcpay_request:ok' ) {
                            alert('支付成功');
                            window.location.href = '/mobile/usertest.php?act=order_list';
                        }
...
elseif ($act == 'act_add_coupons')
{
    ...
    {
        show_message($_LANG['bonus_add_success'], $_LANG['back_up_page'], 'usertest.php?act=coupons', 'info');
    }
    else
    {
...
elseif ($act == 'act_add_gift')
{
    ...
    {
        show_message($_LANG['add_gift_sucess'], $_LANG['back_up_page'], 'usertest.php?act=gift_card', 'info');
    }
    else
    {
        $smarty->assign('err_or', 1);
...
function m_register($username, $password, $email, $other = array())
{
	...
	if (empty($username))
	{
        show_regmessage('用户名不能为空！', '', 'usertest.php?act=register', 'error');
		return false;
	}
	if (preg_match('/\'\/^\\s*$|^c:\\\\con\\\\con$|[%,\\*\\"\\s\\t\\<\\>\\&\'\\\\]/', $username))
	{
        show_regmessage('用户名错误！', '', 'usertest.php?act=register', 'error');
		return false;
	}
...
//	if (empty($email))
//	{
//        show_regmessage('email不能为空！', '', 'usertest.php?act=register', 'error');
//		return false;
//	}
//	if(!is_email($email))
//	{
//        show_regmessage('email错误！', '', 'usertest.php?act=register', 'error');
//		return false;
//	}
	...
	{
        show_regmessage('此用户已存在！', '', 'usertest.php?act=register', 'error');
		return false;
	}
	...
	if (!$GLOBALS['user']->add_user($username, $password, $email))
	{
        ...
        show_regmessage($GLOBALS['err']->_message[0], '', 'usertest.php?act=register', 'error');
		//注册失败
		return false;
	}
	else
