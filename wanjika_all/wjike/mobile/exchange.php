...
setcookie("exchange", "1", time()+3600);
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $goods_name   = isset($_REQUEST['keywords']) && !empty($_REQUEST['keywords']) ? $_REQUEST['keywords'] : "";
    ...
    if (!$smarty->is_cached('exchange.dwt', $cache_id))
    {
        ...
        if(!empty($goods_name))
        {
            $ext = " AND g.goods_name LIKE '%". $goods_name ."%' ";
            $keyword = $goods_name;
        }
...
//        }
    ...
    $is_sign = is_sign();
    //获取连续签到天数
    $sign_day = get_sign_day($is_sign);
    $is_sign = $is_sign ? "已签到" : "签到" ;
    $smarty->assign('is_sign', $is_sign);
    $smarty->assign('sign_day', $sign_day);
    //用户积分
    $pay_points = get_user_points($_SESSION['user_id']);
    $smarty->assign('user_points', $pay_points);
    //签到页面渲染
    $sign_page = false;
    $exchange_name['name'] = "热门兑换";
    $exchange_name['e_name'] = "HOT EXCHANGE";
    $exchange_name['goods_list'] = "商品列表";
    $exchange_name['goods_name'] = "COMMODITY LIST";
    if (isset($_REQUEST['sign']) && $_REQUEST['sign'] == 1)
    {
        $exchange_name['name'] = "热销推荐";
        $exchange_name['e_name'] = "HOT SELLING";
        $sign_page = true;
    }

    $smarty->assign('keywords',           '积分商城');
    $smarty->assign('description',        '积分商城');
    $smarty->assign('exchange_name',$exchange_name);
    $smarty->assign('sign_page',$sign_page);
    ...
    $smarty->display('exchange_list.html');exit;
}
...
elseif ($_REQUEST['act'] == 'sign')
{
    ...
    if($_SESSION[user_id] <= 0)
    {
        $_SESSION['back_act'] = 'exchange.php?sign=1';
        $result['error'] = -2;
        $result['content'] = "未登录";
    ...
    $day = "1";
    //获取签到配置积分
    $sql ="SELECT * FROM " . $GLOBALS['ecs']->table('integral') . " LIMIT 1";
    $integral = $db->getRow($sql);
    //增加基础积分记录account_log: change_type=10为增加签到基础积分，11增加签到额外积分
    log_account_change($user_id,0,0,0,$integral['basic_integral'],"增加签到基础积分",ADD_SURPLUS);
    //update sign_log
    //昨日日期
    $time = time();
    ...
    $continuity_sign = get_sign_day($is_sign);
    $last_sign_time_m = date('Y-m',$last_sign_time);
    $last_sign_time = date('Y-m-d',$last_sign_time);
    if($last_sign_time != "")
    {
        if($data == $last_sign_time)
        {
            $continuity_sign = $continuity_sign + 1;
            $day = "continuity_sign + 1";
        }
        $sql1 ="UPDATE " . $GLOBALS['ecs']->table('sign_log') . " SET  continuity_sign=$day,last_sign_time=$time" . " WHERE user_id='$_SESSION[user_id]'";
    ...
    else
    {
        $sign_log = array(
            ...
            "continuity_sign" => 1,
            "last_sign_time"  => $time,
            "sign_data"       => date('d',$time)
        );
        $continuity_sign = 1;
        $db->autoExecute($ecs->table('sign_log'), $sign_log, 'INSERT');
    }
    if($continuity_sign == 0)
    {
        $continuity_sign = 1;
    }
    //额外赠送积分
    if(!$is_sign && is_int($continuity_sign/$integral['day']))
    {
        log_account_change($user_id,0,0,0,$integral['reward_integral'],"增加签到额外积分",ADD_EXTRA_SURPLUS);
    }
    //签到的日期
    //格式化月份
    $data = date('Y-m',$time);
    //同一个月追加签到日期
    $sql = "SELECT sign_data FROM " . $GLOBALS['ecs']->table('sign_log') . " WHERE user_id='$_SESSION[user_id]'";
    $sign_data = $GLOBALS['db']->getOne($sql);
    if($last_sign_time_m == $data )
    {
        $data_d = date('j',$time);
        if($sign_data =="")
        {
        ...
        {
            $sign_data = $sign_data . ",".$data_d;
            $data_d = ",".$data_d;
        }
        $sql1 ="UPDATE " . $GLOBALS['ecs']->table('sign_log') . " SET sign_data=concat(sign_data,'$data_d')  " . " WHERE user_id='$_SESSION[user_id]'";
        $GLOBALS['db']->query($sql1);
    }
    else
    {
        $data_d = date('j',$time);
        $sign_data = $data_d;
        $sql1 ="UPDATE " . $GLOBALS['ecs']->table('sign_log') . " SET sign_data='$data_d'  " . " WHERE user_id='$_SESSION[user_id]'";
        $GLOBALS['db']->query($sql1);
    }
    $sign_data = explode(",",$sign_data);
    $result['content'] = $sign_data;
    $pay_points = get_user_points($_SESSION['user_id']);
    $result['continuity_sign'] = $continuity_sign;
    $result['user_points'] = $pay_points;
    $result['msg'] = "已签到";
    die($json->encode($result));
    exit;
}
...
function exchange_get_goods($children, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style,g.overseas_logo,gc.country_name, eg.exchange_integral, ' .
...
//        else
        ...
        $arr[$row['goods_id']]['overseas_logo']       = $row['overseas_logo'];
        $arr[$row['goods_id']]['overseas_logo_name']       = $row['country_name'];
