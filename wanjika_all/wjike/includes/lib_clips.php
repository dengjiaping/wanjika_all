...
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.goods_img,g.overseas_logo,gs.is_overseas, g.shop_price AS org_price, '.
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $goods_list[$row['goods_id']]['overseas_logo']    = $row['overseas_logo'];
        $country_array = country_list();
        foreach($country_array AS $key=>$value)
        {
            if($goods_list[$row['goods_id']]['overseas_logo'] == $value['country_code'])
            {
                $goods_list[$row['goods_id']]['overseas_logo_name'] = $value['country_name'];
            }
        }
        ...
        $goods_list[$row['goods_id']]['is_overseas']    = $row['is_overseas'];
        $goods_list[$row['goods_id']]['goods_img']    = $row['goods_img'];
        $goods_list[$row['goods_id']]['market_price']  = price_format($row['market_price']);
        $goods_list[$row['goods_id']]['brief']        = $row['goods_brief'];
        $goods_list[$row['goods_id']]['goods_discount']        = number_format($row['shop_price']/$row['market_price']*10, 1, '.', '');
        ...
        $goods_list[$row['goods_id']]['url']           = 'http://item.wjike.com/'.$row['goods_id'].'.html';
    }
 ...
 */
function get_user_bouns_lists($user_id, $num = 10, $start = 0, $status = 'avl')
{
    $arr = array();
...
//        " WHERE u.bonus_type_id = b.type_id AND u.user_id = '" .$user_id. "'";
    ...
    {
        case 'used' :
            $sql = "SELECT u.bonus_sn, u.order_id, u.use_start_datetime, u.use_end_datetime, b.type_name, b.type_money, b.min_goods_amount, b.use_goods_ids ,b.note FROM " .
                $GLOBALS['ecs']->table('user_bonus'). " AS u ,".
                $GLOBALS['ecs']->table('bonus_type'). " AS b"." WHERE u.bonus_type_id = b.type_id AND u.order_id > 0 and u.user_id = '$user_id'";
            break;
        case 'unavl' :
            $sql = "SELECT u.bonus_sn, u.order_id, u.use_start_datetime, u.use_end_datetime, b.type_name, b.type_money, b.min_goods_amount, b.use_goods_ids ,b.note FROM " .
                $GLOBALS['ecs']->table('user_bonus'). " AS u ,".
            ...
            break;
        default:
            $sql = "SELECT u.bonus_sn, u.order_id, u.bonus_id, u.use_start_datetime, u.use_end_datetime,b.type_name, b.type_money, b.min_goods_amount, b.use_goods_ids ,b.note FROM " .
                $GLOBALS['ecs']->table('user_bonus'). " AS u ,".
                $GLOBALS['ecs']->table('bonus_type'). " AS b".
                " WHERE u.bonus_type_id = b.type_id AND u.use_end_datetime >= $cur_date AND order_id = 0 AND u.user_id = '" .$user_id. "'";
            break;
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if($row['min_goods_amount'] > 0){
            $row['condition'] =  '满'.intval($row['min_goods_amount']).'元减'.intval($row['type_money']).'元';
        }
        elseif(!empty($row['use_goods_ids'])){
            $row['condition'] = '购买指定商品可用';
        }
        else{
            $row['condition'] = '无条件';
        }
 ...
 * @param   int         $user_id         用户ID
 *
 * @return  array       $invoice             发票信息
 */
function get_invoice_info($user_id)
{
    $info = array('type'=>'0','company_info'=>'','content'=>'0');
    $sql  = "SELECT i.* FROM " .$GLOBALS['ecs']->table('invoice'). " AS i, ". $GLOBALS['ecs']->table('users')." AS u".
        " WHERE u.invoice_id = i.id AND u.user_id = '$user_id'";

    $r = $GLOBALS['db']->getAll($sql);
    if(count($r) > 0){
        $info['type'] = $r[0]['type'];
        $info['company_info'] = $r[0]['company_info'];
        $info['content'] = $r[0]['content'];
    }
...
{
    $sql = "SELECT b.card_id,b.card_sn, b.order_id, u.goods_name, b.money, b.leave_money,  b.end_date,b.card_status,b.use_status,b.card_password ".
        " FROM " .$GLOBALS['ecs']->table('gift_card'). " AS b ,".
        $GLOBALS['ecs']->table('goods'). " AS u".
        " WHERE u.goods_id = b.goods_id AND b.member_id = '" .$user_id. "' ORDER BY b.leave_money DESC";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if (empty($row['order_sn']))
        {
            ...
            if ($row['end_date'] < $cur_date)
            {
                ...
                $row['gift_card_status'] = 1;
                $cardStatus = $row['card_status'];
                if($cardStatus ==1 )
                {
                    $row['card_status'] = $GLOBALS['_LANG']['overdue'];
                }
                else
                {
