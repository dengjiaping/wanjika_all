    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                ...
                $cat_arr[$row['cat_id']]['url']  = 'http://www.wjike.com/'.$row['cat_id'].'.html';
                if(is_local()){
    ...
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
               ...
               $three_arr[$row['cat_id']]['url'] = 'http://www.wjike.com/'.$row['cat_id'].'.html';
            if(is_local()){
            ...
            foreach($type2lib as $key => $data)
            {
                if (!empty($goods_data[$key]))
                {
                    ...
                    if ($order_type == 0)
                    {
                        ...
                        $rand_key = array_slice($goods_data[$key], 0);
//                        $rand_key = array_slice($goods_data[$key], 0, $num);
        ...
        $sql = 'SELECT g.goods_id, g.cat_id, g.goods_name,g.overseas_logo, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price,gs.is_overseas, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, RAND() AS rnd " .
                'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
                "LEFT JOIN " . $GLOBALS['ecs']->table('goods_supplier') . " AS gs ON g.supplier_id=gs.type_id ".
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['is_overseas']         = $row['is_overseas'];
            $goods[$idx]['overseas_logo']         = $row['overseas_logo'];

            $country_array = country_list();
            foreach($country_array AS $key=>$value)
            {
                if($goods[$idx]['overseas_logo']  == $value['country_code'])
                {
                    $goods[$idx]['overseas_logo_name'] = $value['country_name'];
                }
            }
...
//            $goods[$idx]['shop_price']   = price_format($row['shop_price']);
            ...
            $goods[$idx]['goods_discount']        = number_format($row['shop_price']/$row['market_price']*10, 1, '.', '');
...
{
    $time = gmtime();
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, gs.is_overseas,gs.is_dds,' .
                'IFNULL(AVG(r.comment_rank), 0) AS comment_rank, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS rank_price,gs.is_overseas " .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
    ...
    if ($row !== false)
    {
        ...
        /* 获得商品的销售价格 */
        $row['goods_discount']        = number_format($row['shop_price']/$row['market_price']*10, 1, '.', '');
        $row['market_ori_price']        = $row['market_price'];
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price,g.overseas_logo,gs.is_overseas, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        $goods[$idx]['promote_price_org'] =  $promote_price;
        $goods[$idx]['is_overseas']         = $row['is_overseas'];
        $goods[$idx]['overseas_logo']         = $row['overseas_logo'];
...
{
    $sql =  'SELECT g.goods_id, g.goods_name, g.market_price,gs.is_overseas,g.overseas_logo, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
...
{
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('venue_manage') ;
    $res = $GLOBALS['db']->getAll($sql);

    foreach ($res AS $row)
    {
            $cat_arr[$row['venue_id']]['id']   = $row['venue_id'];
            $cat_arr[$row['venue_id']]['name'] = $row['venue_name'];
            $cat_arr[$row['venue_id']]['url']  = 'http://www.wjike.com/'.$row['venue_id'].'.html';
            if(is_local()){
                $cat_arr[$row['venue_id']]['url']  = build_uri('venue', array('cid' => $row['venue_id']), $row['venue_id']);
            }
...
/**
 * 创建商品集合
 * @param   int     $collect_name        商品集合名称
 */
function insert_goods_collect($collect_name)
{
    //插入重复数据校验
    $exist_sql="SELECT count(*) FROM " . $GLOBALS['ecs']->table('goods_collect'). " WHERE collect_name='$collect_name'";
    ...
    $addtime = gmtime();
    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_collect') . " (collect_name, add_time) " .
        "VALUES('$collect_name', '$addtime')";
...
/**
 * 删除商品集合
 * @param   int     $gcid        商品集合id
 */
function delete_goods_collect($gcid)
{
    //检查有下属关联商品集合属性不让删除
    $exist_sql="SELECT count(*) FROM " . $GLOBALS['ecs']->table('collect_attr'). " WHERE gcid=$gcid ";
    if($GLOBALS['db']->getOne($exist_sql) > 0)
    {
        return 'error';
    }

    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('goods_collect') . " WHERE gcid = '" . $gcid . "'";
 ...
 * 获取商品集合
 */
function get_goods_collects()
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('goods_collect');
 ...
 * 创建商品集合属性
 * @param   int     $gcid        商品集合id
 * @param   int     $ca_name        商品集合属性
 */
function insert_collect_attr($gcid,$ca_name)
{
    //插入重复数据校验
    $exist_sql="SELECT count(*) FROM " . $GLOBALS['ecs']->table('collect_attr'). " WHERE ca_name='$ca_name' AND gcid=$gcid ";
    if($GLOBALS['db']->getOne($exist_sql) > 0)
    {
        return 'existend';
    }
    ...
    $addtime = gmtime();
    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('collect_attr') . " (gcid, ca_name, add_time) " .
        "VALUES('$gcid', '$ca_name', '$addtime')";
...
/**
 * 删除商品集合属性
 * @param   int     $caid        商品集合属性id
 */
function delete_collect_attr($caid)
{
    //检查有下属关联商品集合属性值不让删除
    $exist_sql="SELECT count(*) FROM " . $GLOBALS['ecs']->table('collect_attrvalue'). " WHERE caid=$caid ";
    if($GLOBALS['db']->getOne($exist_sql) > 0)
    {
        return 'error';
    }

    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('collect_attr') . " WHERE caid = '" . $caid . "'";
 ...
 * @param   int     $gcid        商品集合id
 */
function get_collect_attrs($gcid)
{
    if($gcid <= 0){
        return null;
    }
    $sql = "SELECT caid,ca_name FROM " . $GLOBALS['ecs']->table('collect_attr') .
        " WHERE gcid = '$gcid'";
 ...
 * @param   int     $caid        商品集合属性id
 */
function get_caname_byid($caid)
{
    if($caid <= 0){
        return null;
    }
    $sql = "SELECT ca_name FROM " . $GLOBALS['ecs']->table('collect_attr') .
        " WHERE caid = '$caid'";
 ...
 * 创建商品集合属性
 * @param   int     $caid        商品集合属性id
 * @param   int     $attr_name        商品集合属性值
 */
function insert_collect_attrvalue($caid,$attr_name)
{
    //插入重复数据校验
    $exist_sql="SELECT count(*) FROM " . $GLOBALS['ecs']->table('collect_attrvalue'). " WHERE attr_name='$attr_name' AND caid=$caid ";
    if($GLOBALS['db']->getOne($exist_sql) > 0)
    {
        return 'existend';
    }
    ...
    $addtime = gmtime();
    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('collect_attrvalue') . " (caid, attr_name, add_time) " .
        "VALUES('$caid', '$attr_name', '$addtime')";
...
/**
 * 删除商品集合属性值
 * @param   int     $attrv_id        商品集合属性值id
 */
function delete_collect_attrvalue($attrv_id)
{
    //检查有关联该属性的商品不让删除
    $exist_sql="SELECT count(*) FROM " . $GLOBALS['ecs']->table('goods'). " WHERE CONCAT(',', collect_attrids, ',') LIKE '%," . $attrv_id . ",%'" ;
    if($GLOBALS['db']->getOne($exist_sql) > 0)
    {
        return 'error';
    }

    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('collect_attrvalue') . " WHERE attrv_id = '" . $attrv_id . "'";
 ...
 * @param   int     $caid        商品集合属性id
 */
function get_collect_attrvalues($caid)
{
    if($caid <= 0){
        return null;
    }
    $sql = "SELECT attrv_id,attr_name FROM " . $GLOBALS['ecs']->table('collect_attrvalue') .
        " WHERE caid = '$caid'";
 ...
 * @param   int     $attrv_id        商品集合属性值id
 */
function get_attrvalue_byid($attrv_id)
{
    if($attrv_id <= 0){
        return null;
    }
    $sql = "SELECT attr_name FROM " . $GLOBALS['ecs']->table('collect_attrvalue') .
        " WHERE attrv_id = '$attrv_id'";
...
/**
 * 获取商品集合属性值(以逗号分隔的字符串值)
 * @param   int     $gcid        商品集合id
 */
function get_str_collect_attrvalues($caid)
{
    if($caid <= 0){
        return null;
    }
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('collect_attrvalue') .
        " WHERE caid = '$caid'";
 ...
 * @param   int     $goods_id        商品id
 * @param   int     $gcid        商品集合id
 * @param   int     $collect_attrids   商品属性值
 */
function set_in_collect($goods_id, $gcid, $collect_attrids,$is_add=0)
{
    //插入重复数据校验
    $exist_sql="SELECT count(*) FROM " . $GLOBALS['ecs']->table('goods'). " WHERE collect_attrids='$collect_attrids' AND gcid=$gcid AND goods_id<>$goods_id" ;
    if($GLOBALS['db']->getOne($exist_sql) > 0)
    {
        return 'existend';
    }
    if($is_add>0)
    {
        $exist_sql1="SELECT count(*) FROM " . $GLOBALS['ecs']->table('goods'). " WHERE gcid>0 AND goods_id=$goods_id" ;
        if($GLOBALS['db']->getOne($exist_sql1) > 0)
        {
            return 'existend';
        }
    }
    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('goods') .
        " SET collect_attrids='$collect_attrids',gcid=$gcid WHERE goods_id=$goods_id";
 ...
 * @param   int     $goods_id        商品id
 */
function set_out_collect($goods_id)
{
    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('goods') .
        " SET collect_attrids='',gcid=0 WHERE goods_id=$goods_id";
 ...
 * @param   int     $goods_id        商品id
 */
function get_collect_goods($gcid,$goods_id=0)
{
    if($gcid <= 0){
        return null;
    }
    $sql = "SELECT goods_id,goods_name,collect_attrids FROM " . $GLOBALS['ecs']->table('goods') .
        " WHERE gcid = '$gcid'";
    if($goods_id > 0){
        $sql = "SELECT goods_id,goods_name,collect_attrids FROM " . $GLOBALS['ecs']->table('goods') .
            " WHERE gcid = '$gcid' AND goods_id<>$goods_id";
    }
    ...
    $array_attrs = $GLOBALS['db']->getAll($sql);
    ...
    foreach($array_attrs as $k=>$v){
        ...
        $sql = "SELECT attrv_id,attr_name FROM " . $GLOBALS['ecs']->table('collect_attrvalue') .
            " WHERE caid = '$caid'";
        $attrvid_array = array();
        foreach($GLOBALS['db']->getAll($sql) as $value){
            array_push($attrvid_array,$value["attrv_id"]);
        }
        $array_attrs[$k]['attrvid'] = $attrvid_array;
    }

    foreach($r as $k=>$v){
        $collect_attrids = explode(',',$v["collect_attrids"]);
        $array_attr = $array_attrs;
        foreach($array_attr as $key=>$value){
            foreach($collect_attrids as $attrid){
                if(in_array($attrid,$value['attrvid'])){
                    $array_attr[$key]['attrvid'] = $attrid;
                    $gattr = get_attrvalue_byid($attrid);
                    $array_attr[$key]['attr_value'] = $gattr;
                }
               if($array_attr[$key]['attr_value'] ==null)
               {
                   $array_attr[$key]['attr_value'] = "无";
               }
            }
        }
        $r[$k]['attr_array'] = $array_attr;
    }
 ...
 * @param   int     $gcid        商品集合id
 */
function get_goods_collectvalues($goods_id)
{
    if($goods_id <= 0){
        return null;
    }
    $sql = "SELECT collect_attrids,gcid FROM " . $GLOBALS['ecs']->table('goods') .
        " WHERE goods_id = '$goods_id'";

    $results = $GLOBALS['db']->getRow($sql);
    $gcid = $results["gcid"];
    if($gcid <= 0){
        return null;
    }
    ...
    foreach($array_attrs as $k=>$v){
        ...
        $attrvid_array = array();
        foreach($GLOBALS['db']->getAll($sql) as $value){
            $attrv_array[$value["attrv_id"]] = $value["attr_name"];
            array_push($attrvid_array,$value["attrv_id"]);
        }
        $array_attrs[$k]['attrv_array'] = $attrv_array;
        $array_attrs[$k]['goods_attrvalue'] = $attrvid_array;
    }

    $collect_attrids = explode(',',$results["collect_attrids"]);
    foreach($array_attrs as $key=>$value){
        foreach($collect_attrids as $attrid){
            if(in_array($attrid,$value['goods_attrvalue'])){
                $gattr = get_attrvalue_byid($attrid);
                $array_attrs[$key]['goods_attrid'] = $attrid;
                $array_attrs[$key]['goods_attrvalue'] = $gattr;
            }
            if($attrid ==null)
            {
                $array_attrs[$key]['goods_attrvalue'] = "无";
            }
        }
    }
...
/**
 * 根据商品d获取商品集合属性
 * @param   string     $collect_attrids        商品集合属性值id
 */
function get_goodsid_by_cattr($collect_attrids)
{
    $sql="SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods'). " WHERE collect_attrids='$collect_attrids'" ;
 ...
 * 显示一品多类信息页面
 */
function assign_collect_attr_info($goods_id)
{
    if($goods_id <= 0){
        return null;
    }
    //获取所有组名
    $get_all_collect = get_goods_collects();

    $GLOBALS['smarty']->assign('all_collect', $get_all_collect);
    $sql = "SELECT collect_attrids,gcid FROM " . $GLOBALS['ecs']->table('goods') .
        " WHERE goods_id = '$goods_id'";
    ...
    $GLOBALS['smarty']->assign('gcid', $gcid);

    $sql = "SELECT collect_name FROM " . $GLOBALS['ecs']->table('goods_collect') .
        " WHERE gcid = '$gcid'";

    $collect_name = $GLOBALS['db']->getOne($sql);

    $GLOBALS['smarty']->assign('collect_name', $collect_name);
    ...
    foreach($collect_attr as $k=>$v){
        ...
        $collect_attr[$k]['goods_attrvalue'] = $attrvid_array;
    }

    $collect_attrids = explode(',',$results["collect_attrids"]);
    foreach($collect_attr as $key=>$value){
        $flag = true;
        foreach($collect_attrids as $attrid){
            if(in_array($attrid,$value['goods_attrvalue'])){
                $gattr = get_attrvalue_byid($attrid);
                $collect_attr[$key]['goods_attrvalue'] = $gattr;
                $flag = false;
            }
        }
        if($flag){
            $collect_attr[$key]['goods_attrvalue'] = '无';
        }
    }
    $GLOBALS['smarty']->assign('collect_attr', $collect_attr);
    $GLOBALS['smarty']->assign('collect_goods', get_collect_goods($gcid,$goods_id));
}
?>