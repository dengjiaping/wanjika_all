 ...
 * @return  true
 */
function check_bank_money($log_id, $money)
{
    if(is_numeric($log_id))
    {
        $sql = 'SELECT order_amount FROM ' . $GLOBALS['ecs']->table('pay_log') .
...
//                  file_put_contents('yiqifa.txt',$cookie_yqf);
                    $cookie_yqf = explode(':',$cookie_yqf);
                    if($cookie_yqf[2] == 18432)
                    {
                        include_once('advertiser/TestOrderStatus.php');
                    }
                }
                /*********增加亿起发修改订单代码end*********/
                /* 记录订单操作记录 */
                ...
                if (!empty($virtual_goods) && key($virtual_goods) != 'goods_members')
                {
                    $msg = '';
                ...
                elseif(key($virtual_goods) == 'goods_members'){
                    $virtual_goods["goods_members"][0]['user_id'] = $user_id;

                    if (!members_charge_ship($virtual_goods))
                    {
                        $GLOBALS['_LANG']['pay_success'] .= '会员充值失败';
                    }
                    else{
                        /* 修改订单状态 */
                        update_order($order_id, array('shipping_status' => SS_RECEIVED, 'order_status' => OS_SPLITED, 'shipping_time' => gmtime()));
                    }
                }
        ...
        else
        {
            ...
            /* 有已发货的虚拟商品 */
            if (!empty($post_virtual_goods) && key($virtual_goods) != 'goods_members')
            {
                $msg = '';
                /* 检查两次刷新时间有无超过12小时 */
 ...
 * @param   float    $trade_no       交易流水号
 * @return  true
 */
function update_trade_no($order_sn, $trade_no)
{
    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_info') .
        " SET pay_trade_no = '$trade_no',kjt_pay_tradeno = '$trade_no' WHERE order_sn = '$order_sn'";
