...
//    $sql = 'SELECT SUM(c.goods_number) AS number, SUM(c.goods_price * c.goods_number) AS amount' .
...
//           " AS gs ON g.supplier_id=gs.type_id WHERE c.session_id = '" . SESS_ID . "' AND c.rec_type = '" . CART_GENERAL_GOODS . "' AND gs.is_overseas=0";
    $sql = 'SELECT c.goods_number,c.goods_id,c.extension_code,g.max_number,g.promote_price,g.promote_start_date,g.promote_end_date, g.shop_price AS goods_price ' .
        ' FROM ' . $GLOBALS['ecs']->table('cart') .
        " AS c LEFT JOIN ". $GLOBALS['ecs']->table('goods') . " AS g ON c.goods_id=g.goods_id WHERE c.session_id = '" . SESS_ID . "' AND c.rec_type = '" . CART_GENERAL_GOODS. "' AND (g.is_immediately=0 or c.extension_code='package_buy')" ;
    ...
    if ($row)
    {
        foreach ($row as $key => $value)
        {
            if($value["extension_code"] == "package_buy"){
                ...
                $value["goods_price"] = $package_price;
                $value["shop_price"] = $package_price;
                $value["max_number"] = "1";
            }
            $value['goods_price'] = get_final_price($value['goods_id'], $value['goods_number'], true, $spec = array());
            /* 修正促销价格 */
            if ($value['promote_price'] > 0)
            {
                ...
                if($promote_price>0)
                {
                    $value['goods_price'] = $value['promote_price'];
                }
            }
            if($value['max_number']!=0)
            {
                ...
                {
                    $amount     += $value['goods_price']*$value['max_number'];
...
function insert_member_info()
{
    ...
    if ($_SESSION['user_id'] > 0)
    {
        ...
        $GLOBALS['smarty']->assign('is_members', $is_members);
    }
    else
    {
