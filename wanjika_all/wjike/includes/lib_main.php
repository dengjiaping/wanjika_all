...
function update_user_info()
{
    ...
    $sql = 'SELECT u.user_money,u.email, u.pay_points, u.user_rank, u.rank_points, '.
            ...
            " ON b.type_id = ub.bonus_type_id AND ub.use_start_datetime <= '$time' AND ub.use_end_datetime >= '$time' ".
...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    $ur_here    = '<a href="http://www.wjike.com">' . $GLOBALS['_LANG']['home'] . '</a>';
        ...
        if (in_array($filename, array('category', 'goods', 'article_cat', 'article', 'brand')))
        {
            ...
            if (!empty($cat_arr))
            {
                ...
                foreach ($cat_arr AS $val)
                {
                    ...
                    if ('goods' == $filename)
                    {
                        $url = 'http://www.wjike.com/'.$val['cat_id'].'.html';
                        $ur_here   .= ' <code>&gt;</code> <a href="' . $url . '">' .
                    ...
                    $ur_here_name=$val['cat_name'];
                    $ur_here_id=$val['cat_id'];
                }
            }
        }
        /* 处理无分类的 */
    ...
    /* seo列表页title规范 */
    if (in_array($filename, array('category')))
    {
        $parent_id = $GLOBALS['db']->GetOne('SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category').' where cat_id='.$cat);
        if($parent_id === '0'){
            $page_title  = '进口'.htmlspecialchars($val['cat_name']) .'_进口'.htmlspecialchars($val['cat_name']) .'商城_网上买进口'.htmlspecialchars($val['cat_name']) .'上万集客【网购 价格 图片 品牌】-万集客商城';
        }
        else{
            $page_title  = '进口'.htmlspecialchars($val['cat_name']) .'_进口'.htmlspecialchars($val['cat_name']) .'优惠_进口'.htmlspecialchars($val['cat_name']) .'在线购买【网购 价格 图片 品牌】-万集客商城';
        }
    }
    if (in_array($filename, array('goods')))
    {
        $page_title  = $str .'【优惠、折扣、价格、图片】- 万集客官网';
    }

    /* 返回值 */
    return array('title' => $page_title, 'ur_here' => $ur_here,'ur_here_name'=>$ur_here_name,'ur_here_id'=>$ur_here_id);
}
...
function get_shop_help()
{
    ...
    foreach ($res AS $key => $row)
    {
        ...
        $arr[$row['cat_id']]['article'][$key]['url']         = $row['open_type'] != 1 ?
            'http://help.wjike.com/'.$row['article_id'].'.html' : trim($row['file_url']);
        if(is_local()){
        $arr[$row['cat_id']]['article'][$key]['url']         = $row['open_type'] != 1 ?
 ...
 * @return  void
 */
function show_message($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true, $trans_flag = '')
{
    assign_template();
    ...
    if (!empty($trans_flag))
    {
        $GLOBALS['smarty']->assign('trans_flag', $trans_flag); // 跳转页面标识
    }

    $GLOBALS['smarty']->assign('message', $msg);
    $GLOBALS['smarty']->assign('auto_redirect', $auto_redirect);
    $GLOBALS['smarty']->assign('user_id', (string)$_SESSION['user_id']);
