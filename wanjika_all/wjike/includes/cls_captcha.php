...
//    );
     ...
     * @access  public
     * @param   string  $word   验证码
     * @param   string  $admintrue   区分后台和前台的验证码样式
     * @return  mix
     */
    function generate_image($word = false,$admintrue=false)
    {
        if (!$word)
        {
        ...
        else
        {
            ...
            $img_org   = ((function_exists('imagecreatetruecolor')) && PHP_VERSION >= '4.3') ?
                          imagecreatetruecolor($this->width, 30) : imagecreate($this->width, $this->height);

            /* 将背景图象复制原始图象并调整大小 */
            if (function_exists('imagecopyresampled') && PHP_VERSION >= '4.3') // GD 2.x
            {
                imagecopyresampled($img_org, $img_bg, 0, 0, 0, 0, $this->width, 30, $bg_width, $bg_height);
            }
            else // GD 1.x
            {
                imagecopyresized($img_org, $img_bg, 0, 0, 0, 0, $this->width, 30, $bg_width, $bg_height);
            }
            imagedestroy($img_bg);
            ...
            if($admintrue ==true)
            {
                $y = (30 - imagefontheight(5)) / 2;
//                imagestring($img_org, 5, $x, $y, $word, $clr);
                ImageTTFText($img_org, 15, rand(0, 10), 50, 25, $clr, "../data/captcha/arial.ttf", $word);
                $t = imagecolorallocate($img_org, rand(0, 255),rand(0, 255),rand(0, 255));
                // 为图片添加噪点
                for($i=0; $i<200; $i++)
                {
                    imagesetpixel($img_org, rand(1, 145), rand(1, 40), $t);
                }
            }
            else
            {
                $y = (40 - imagefontheight(5)) / 2;
                ImageTTFText($img_org, 15, rand(0, 10), 25, 25, $clr, ROOT_PATH . "data/captcha/arial.ttf", $word);
                for($i=0; $i<5; $i++)
                {
                    // 画线
                    imageline($img_org, 0, rand(0, 30), rand(70,100), rand(0, 30), $clr);
                }
                $t = imagecolorallocate($img_org, rand(0, 255),rand(0, 255),rand(0, 255));
                // 为图片添加噪点
                for($i=0; $i<200; $i++)
                {
                    imagesetpixel($img_org, rand(1, 100), rand(1, 40), $t);
                }
            }
