...
class alipay
{
     ...
     * @param   array   $payment    支付方式信息
     */
    function get_paycode($order, $payment,$type = 'web')
    {
        $button = '<form target="_blank" name="payment" method="post" action="user.php">';
        $button .= "<input type='hidden' name='pay_id' value='" . $order['pay_id'] . "'>";
        $button .= "<input type='hidden' name='order_id' value='" . $order['order_id'] . "'>";
        $button .= "<input type='hidden' name='act' value='act_edit_payment' />";
        $button .= "<input class='order_pay' type='submit' value='去支付' name='Submit' onclick='add();'>";
    ...
    function redirect_pay($order, $payment,$type = 'web')
    {
        ...
        $notify_url = 'http://www.wjike.com/pay/aliRcvPay.php';
        $return_url = $GLOBALS['ecs']->url() . 'user.php?act=order_list';
        if($type == 'web'){
            $notify_url = $GLOBALS['ecs']->url() . 'pay/aliRcvPay.php';
...
//            'logistics_payment' => 'BUYER_PAY_AFTER_RECEIVE',
        ...
        $def_url  = '<form name="alipay" style="text-align:center;" method="post"'.
        ...
        $def_url .= "</form><script>document.forms['alipay'].submit();</script>";
