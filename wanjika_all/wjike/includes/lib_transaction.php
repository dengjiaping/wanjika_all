...
function get_profile($user_id)
{
    ...
    $sql = "SELECT type_name, type_money ".
           ...
           "WHERE t1.type_id = t2.bonus_type_id AND t2.user_id = '$user_id' AND t2.use_start_datetime <= '$cur_date' ".
           "AND t2.use_end_datetime > '$cur_date' AND t2.order_id = 0";
...
function add_bonus($user_id, $bouns_sn)
{
    ...
    /* 查询红包序列号是否已经存在 */
    $sql = "SELECT u.bonus_id, u.bonus_sn, u.user_id, u.bonus_type_id,b.use_start_date,b.use_end_date FROM " .$GLOBALS['ecs']->table('user_bonus') . " AS u ,".
            $GLOBALS['ecs']->table('bonus_type'). " AS b".
           " WHERE u.bonus_type_id = b.type_id AND bonus_sn = '$bouns_sn'";
    ...
    if ($row)
    {
        ...
        {
            //红包没有被使用
            $sql = "SELECT send_end_date,use_start_date, use_end_date ,send_type ,is_limit".
            ...
            if($bonus_time['is_limit'] == 1)
            {
                ...
                {
                    $GLOBALS['err']->add($GLOBALS['_LANG']['bonus_is_use']);
                    return false;
                }
            }
            $sql = "UPDATE " .$GLOBALS['ecs']->table('user_bonus') . " SET user_id = '$user_id',binding_time = '$now',use_start_datetime = '$row[use_start_date]',use_end_datetime = '$row[use_end_date]' ".
 ...
 */
function get_user_orders($user_id, $num = 10, $start = 0,$where = ' and 1')
{
    /* 取得订单列表 */
    $arr    = array();

    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status,consignee, pay_id, pay_name,add_time,extension_code, " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            //已确认修改为不显示已确认字样
            $row['handler'] = $row['order_status'] != 1 ? $GLOBALS['_LANG']['os'][$row['order_status']] : "";
        }
        ...
        $is_members = false;
        $sql1 = "SELECT o.*, g.goods_thumb,g.shop_price " .
            " FROM " .$GLOBALS['ecs']->table('order_goods'). " AS o ".
            "LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ON o.goods_id = g.goods_id " .
            " WHERE o.order_id = '$order_id' ORDER BY rec_id DESC";
        $res1 = $GLOBALS['db']->query($sql1);
        $row['goods_info']=array();
        while ($row1 = $GLOBALS['db']->fetchRow($res1)){
            if($row1['extension_code'] == 'goods_members'){
                $is_members = true;
            }
            $row['goods_info'][]=array('goods_name'       => $row1['goods_name'],
                'goods_id'       => $row1['goods_id'],
                'shop_price'       => price_format($row1['goods_price'],false),
                'is_real'       => $row1['is_real'],
                'goods_number'       => $row1['goods_number'],
                'goods_thumb'       => $row1['goods_thumb']);
        }
        //如果是积分订单
        if($row['extension_code'] == 'exchange_goods')
        {
            $row['pay_name'] = "积分";
        }
        $arr[] = array('order_id'       => $row['order_id'],
                       ...
                       'order_status'   => $row['order_status'],
                        'ori_order_status'   => $order_status,
                        'pay_status'   => $row['pay_status'],'is_members'      => $is_members,
            'shipping_status'   => $row['shipping_status'],
                       'order_statusid'   => $order_status,
                       'order_payid'   => $row['pay_id'],
                       'order_paystatus'   => $row['pay_status'],
                       'order_pay'      => $row['pay_name'],
                       'order_consignee' => $row['consignee'],
                       'order_goods'    =>$row['goods_info'],
...
{
    /* 查询订单信息，检查状态 */
    $sql = "SELECT user_id, order_id, order_sn , surplus , integral , gift_card_id , gift_money, bonus_id, order_status, shipping_status, pay_status, is_overseas FROM " .$GLOBALS['ecs']->table('order_info') ." WHERE order_id = '$order_id'";
    ...
    if($order['is_overseas']){
        ...
        $overseas_sql = "UPDATE ".$GLOBALS['ecs']->table('kjt_task') ." SET task_status = 13 WHERE order_no = '$order_no'";
        $GLOBALS['db']->query($overseas_sql);
    }
    if ($GLOBALS['db']->query($sql))
    {
        ...
        if ($order['user_id'] > 0 && $order['gift_card_id'] > 0)
        {
            change_user_gift($order['gift_card_id'], $order['order_id'],$order['gift_money'], false);
        }
...
function save_consignee($consignee, $default=false)
{
    ...
    else
    {
        ...
        $_SESSION['session_address_id']=$consignee['address_id'];
    }
...
function update_address($address)
{
    ...
    else
    {
        ...
        /* 结算页面增加地址后默认选中新增地址，所有保存到session */
        $_SESSION['flow_consignee'] = $consignee;
    }
 ...
 */
function get_order_detail($order_id, $user_id = 0,$type = 'web')
{
    ...
    $order['shipping_no'] = $order['invoice_no'];
        ...
        $order['order_status'] == OS_CONFIRMED))
    {
        ...
        else
        {
            ...
            {
                $order['pay_online'] = $pay_obj->get_code($order, $payment,$order['inv_content'],$bank_code_arr);
            }
            else
            {
                $order['pay_online'] = $pay_obj->get_code($order, $payment, $type);
            }
            if ($payment_info['pay_code'] == 'weixinpay')
            {
                $openid = $pay_obj->get_Openid();
                $order['jspara'] = $pay_obj->get_jspara($order, $payment, $openid);
            }
        }
    }
    else
...
{
    $sql = "SELECT u.bonus_sn, u.order_id, b.type_name, b.type_money, b.min_goods_amount, u.use_start_datetime, u.use_end_datetime ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            /* 没有被使用 */
            if ($row['use_start_datetime'] > $cur_date)
            {
                $row['status'] = $GLOBALS['_LANG']['not_start'];
            }
            else if ($row['use_end_datetime'] < $cur_date)
            {
        ...
        $row['use_startdate']   = local_date($GLOBALS['_CFG']['date_format'], $row['use_start_datetime']);
        $row['use_enddate']     = local_date($GLOBALS['_CFG']['date_format'], $row['use_end_datetime']);
...
function get_shipping_code($getcom){
    switch ($getcom){
