...
function get_brands($cat = 0, $app = 'brand')
{
    ...
    $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, b.brand_desc,b.brand_desc_img,b.overseas_logo, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag ".
 ...
 *
 * @access  public
 * @param   array  $members_charge   会员充值商品数组
 *
 * @return bool
 */
function members_charge_ship(&$goods_members, $return_result = false)
{
    $members_charge = array();
    foreach ($goods_members AS $code => $list)
    {
        if ($code == 'goods_members')
        {
            foreach ($list as $info)
            {
                if (being_members($info))
                {
                    if ($return_result)
                    {
                        $members_charge[] = array('goods_id'=>$info['goods_id'], 'user_id' => $info['user_id']);
                    }
                }
                else
                {
            ...
            $GLOBALS['smarty']->assign('goods_members', $members_charge);
        }
    }
 ...
 * @return  void
 */
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER,$integrals = 1,$change_order_sn=0)
{
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
        ...
        'change_order_sn'=> $change_order_sn,
        'change_multiple'=> $integrals
    );
 ...
 * @return  void
 */
function is_send_integral($order_sn)
{
    $sql = 'SELECT COUNT(*) " .
               " FROM ' . $GLOBALS['ecs']->table('account_log') .
        " WHERE change_order_sn= '$order_sn' AND pay_points>0 " ;
...
{
    $sql = "SELECT pg.goods_id, g.goods_name,g.goods_thumb, pg.goods_number,ga.act_name,ga.ext_info, p.goods_attr, p.product_number, p.product_id
            FROM " . $GLOBALS['ecs']->table('package_goods') . " AS pg
                LEFT JOIN " .$GLOBALS['ecs']->table('goods_activity') . " AS ga ON pg.package_id = ga.act_id
    ...
    foreach ($row as $key => $value)
    {
        ...
        else
        {
            ...
            $row[$key]['goods_name2'] = $value['goods_name'];
        }
    }
 ...
 * @return  会员等级信息
 */
function get_all_user_members($price)
{
    $result = null;
    $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('user_members'). "".
        " WHERE `price` = '$price'";
 ...
 * @return  会员等级信息
 */
function get_user_members($goods_id)
{
    $result = null;
    $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('user_members'). "".
        " WHERE `goods_id` = '" . $goods_id . "'";
 ...
 *
 * @access  public
 * @param   string      $info      信息详情数组
 *
 * @return  boolen
 */
function being_members ($info)
{
    $user_members = get_user_members($info['goods_id']);
    if($user_members == null){
        return false;
    }
    $members_id = $user_members['id'];
    $user_id = $info['user_id'];
    $members_deadline = gmtime() + ($user_members['days'] * 24 * 3600);

    $sql  = "UPDATE " .$GLOBALS['ecs']->table('users'). " SET is_members='1', members_id='$members_id', members_deadline='$members_deadline'".
 ...
 *
 */
function get_member_integralrate($user_id)
{
    $sql="SELECT integralrate FROM " .$GLOBALS['ecs']->table('users') . " t1 " .
        " LEFT JOIN " .$GLOBALS['ecs']->table('user_members') . " t2 ON t1.members_id=t2.id WHERE user_id='$user_id' AND t1.is_members=1 AND t1.members_id<>0 ";
 ...
 *
 */
function check_user_members($user_id)
{
    $result = false;
    $sql = "SELECT is_members,members_deadline FROM " .$GLOBALS['ecs']->table('users'). "".
        " WHERE `user_id` = '" . $user_id . "'";

    $res = $GLOBALS['db']->getRow($sql);
    if(!empty($res) && $res["is_members"] == 1 && $res["members_deadline"] >= gmtime()){
        $result = true;
    }
 ...
 * @return  boolen
 */
function members_overtime ($user_id)
{
    $sql  = "UPDATE " .$GLOBALS['ecs']->table('users'). " SET is_members='0', members_id='0'".
        " WHERE user_id='" . $user_id . "'";
    $result = $GLOBALS['db']->query($sql);
    if(!$result){
        return false;
 ...
 *
 */
function check_free_members($user_id = 0)
{
    $result = true;
    $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('freemembers_log'). "".
        " WHERE has_get = '1' and user_id = '" . $user_id . "'";
 ...
 * @return  会员等级信息
 */
function get_members_byuser($user_id)
{
    $result = null;
    $msql = "SELECT * FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id = '$user_id'";
    $mres = $GLOBALS['db']->getRow($msql);
    if(!$mres){
        return $result;
    }
    //免费领取会员用户
    if($mres['members_id'] == 0 && $mres['is_members'] == 1){
        $result['goods_name'] = '体验会员';
        $result['members_deadline'] = $mres['members_deadline'];
    }
    else{
        $sql = "SELECT g.goods_name,u.members_deadline
            FROM " . $GLOBALS['ecs']->table('users') . " AS u INNER JOIN " .$GLOBALS['ecs']->table('user_members') . " AS m ON u.members_id=m.id
            INNER JOIN " .$GLOBALS['ecs']->table('goods') . " AS g ON g.goods_id=m.goods_id  WHERE u.user_id = '$user_id'";
 ...
 * 给新注册用户发放优惠券
 */
function send_bonuns($user_id)
{
    //红包类型ID
    $bonus_typeid = 37;
    //判断发放时间
    $rel = $GLOBALS['db']->getRow("SELECT send_start_date,send_end_date FROM ". $GLOBALS['ecs']->table('bonus_type')." where type_id=$bonus_typeid");
    if(gmtime() < $rel["send_end_date"] && gmtime() > $rel["send_start_date"]){
        /* 生成红包序列号 */
        $num = $GLOBALS['db']->getOne("SELECT MAX(bonus_sn) FROM ". $GLOBALS['ecs']->table('user_bonus'));
        $num = $num ? floor($num / 10000) : 100000;
        $now = gmtime();
        $bonus_sn = ($num + mt_rand(0, 9)) . str_pad(mt_rand(0, 9999), 4, '0', STR_PAD_LEFT);
        $sql1 = "SELECT * FROM " . $GLOBALS['ecs']->table('bonus_type') .
            " WHERE type_id = '$bonus_typeid'";
        $bonus =  $GLOBALS['db']->getRow($sql1);
        $GLOBALS['db']->query("INSERT INTO ".$GLOBALS['ecs']->table('user_bonus')." (bonus_type_id, bonus_sn, user_id,binding_time,use_start_datetime,use_end_datetime) VALUES('$bonus_typeid', '$bonus_sn', '$user_id','$now','$bonus[use_start_date]','$bonus[use_end_date]')");
...
{
    $sql = 'SELECT type_id, warehouse_type FROM ' . $GLOBALS['ecs']->table('goods_supplier') . ' ORDER BY type_id';
    $res = $GLOBALS['db']->getAll($sql);

    $supplier_list = array();
    foreach ($res AS $row)
    {
        $supplier_list[$row['type_id']] = addslashes($row['warehouse_type']);
    }
 ...
 * 取得所有erp分类
 */
function erpcat_list()
{
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('erp_category');
    $res = $GLOBALS['db']->getAll($sql);
    return $res;
}
...
{
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('goods_supplier');
    $res = $GLOBALS['db']->getAll($sql);

    foreach ($res AS $key=>$val)
    {
        switch($val["shipping_id"])
        {
            case 1:
                $ShipName="顺丰快递";
                break;
        ...
        $res[$key]['shipping_name'] = $ShipName;
    }
...
{
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('goods_country');
    $res = $GLOBALS['db']->getAll($sql);
    return $res;
}
function is_local()
{
    ...
    if(strpos($_SERVER["HTTP_HOST"],'192.168.11') !== false)
    {
        $result = true;
    }
    if(strpos($_SERVER["HTTP_HOST"],'192.168.2') !== false)
    {
        $result = true;
    }
...
function is_sign()
{
    ...
    //查询当前用户上次签到时间
    $sql = "SELECT last_sign_time FROM " . $GLOBALS['ecs']->table('sign_log') . " WHERE user_id='$_SESSION[user_id]'";
    $last_sign_time = $GLOBALS['db']->getOne($sql);
    //格式化最后签到时间
    $last_sign_time = date('Y-m-d',$last_sign_time);
...
//获取连续签到天数
function get_sign_day($is_sign)
{
    //昨日日期
    $data = date('Y-m-d',time()-86400);
    $sql = $sql = "SELECT continuity_sign FROM " . $GLOBALS['ecs']->table('sign_log') . " WHERE user_id='$_SESSION[user_id]'";
    //已签到
    $continuity_sign = $GLOBALS['db']->getOne($sql);
    if(!$is_sign)
    //未签到
    {
        //查询当前用户上次签到时间
