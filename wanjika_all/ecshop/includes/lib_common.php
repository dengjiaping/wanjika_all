 ...
 *
 * @access  public
 * @param   array  $charge_goods   话费商品数组
 ...
 * @return bool
 */
function charge_goods_ship(&$charge_goods, &$msg, $order_sn, $return_result = false, $process = 'other')
{
    $charge_calls = array();
    foreach ($charge_goods AS $code => $goods_list)
    {
        if ($code == 'charge_calls')
        {
            foreach ($goods_list as $goods)
            {
                if (charge_calls_shipping($goods, $order_sn, $msg, $process))
                {
                    if ($return_result)
                    {
                        $charge_calls[] = array('goods_id'=>$goods['goods_id'], 'goods_name'=>$goods['goods_name'], 'charge_amount' => $goods['charge_amount'], 'tel' => $goods['tel']);
                    }
                }
                else
                {
                    $error_msg = $GLOBALS['_LANG']['charge_calls_fail'];
                    $GLOBALS['smarty']->assign('charge_calls_failed', $error_msg);
                    return false;
                }
            }
            $GLOBALS['smarty']->assign('charge_calls',      $charge_calls);
        }
    }
 ...
 * @return bool
 */
function gift_goods_ship(&$gift_goods, &$msg, $order_sn, $return_result = false, $process = 'other')
{
    $gift_card = array();
    foreach ($gift_goods AS $code => $goods_list)
    {
        /* 只处理虚拟卡 */
        if ($code == 'gift_card')
        {
            foreach ($goods_list as $goods)
            {
                if (gift_card_shipping($goods, $order_sn, $msg, $process))
                {
                    if ($return_result)
                    {
                        $gift_card[] = array('goods_id'=>$goods['goods_id'], 'goods_name'=>$goods['goods_name'], 'info'=>gift_card_result($order_sn, $goods));
                        //print_r($gift_card);
                    }
                }
                else
 ...
 * @return bool
 */
function phone_goods_ship(&$phone_goods, &$msg, $order_sn, $return_result = false, $process = 'other')
{
    $phone_card = array();
    foreach ($phone_goods AS $code => $goods_list)
    {
        /* 只处理虚拟卡 */
        if ($code == 'phone_card')
        {
            foreach ($goods_list as $goods)
            {
                if (phone_card_shipping($goods, $order_sn, $msg, $process))
                {
                    if ($return_result)
                    {
                        $phone_card[] = array('goods_id'=>$goods['goods_id'], 'goods_name'=>$goods['goods_name'], 'info'=>phone_card_result($order_sn, $goods));
                    }
                }
                else
                {
 ...
 * @return  boolen
 */
function charge_calls_shipping ($goods, $order_sn, &$msg, $process = 'other')
{
    /* 包含加密解密函数所在文件 */
    include_once(ROOT_PATH . 'includes/cls_charge.php');
    $charge = new charge();
    $r = $charge->beginfill($order_sn,$goods['tel'],$goods['charge_amount']);
    if($r['status'] == true){
        if($r['leftmoney'] < 100){
            set_unsale();
        }
...
function virtual_card_shipping ($goods, $order_sn, &$msg, $process = 'other')
{
    ...
    include_once(ROOT_PATH . 'includes/cls_mysms.php');
    ...
    {
        /* 获取订单信息 */
        $sql = "SELECT order_id, order_sn, consignee, email, tel, user_id FROM ".$GLOBALS['ecs']->table('order_info'). " WHERE order_sn = '$order_sn'";
    ...
    foreach($cards as $card){
        $card_info .= '卡号：'.$card['card_sn'].' 密码：'.$card['card_password'].'。';
    }
    $sms_content = '尊敬的用户'.$order['consignee'].',您的订单'.$order['order_sn'].'中购买了'.$goods['goods_name'].'共计'
        .$goods['num'].'张。'.$card_info.' 万集客商城 '.date('Y-m-d');
    $mysms = new mysms();
    $r = $mysms->send($order['tel'],$sms_content,'sdkapt',1);
    $result = '成功';
    $status = 1;
    if(!$r['status']){
        $result = '发送失败。错误信息为api_errors:'. $r['api_errors'] . '。server_errors:'. $r['server_errors'];
        $status = 0;
    }
    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('sms_sendresult') . "(user_id,user_name,tel,content,card_type,status,send_result)
    VALUES ('" . $order['user_id'] . "', '" . $order['consignee'] . "', '" . $order['tel'] . "', '" . $sms_content . "', '" . "virtual_card". "', '" . $status . "', '" . $result. "')";
    $GLOBALS['db']->query($sql);
 ...
 * @return  boolen
 */
function gift_card_shipping ($goods, $order_sn, &$msg, $process = 'other')
{
    /* 包含加密解密函数所在文件 */
    include_once(ROOT_PATH . 'includes/lib_code.php');
    include_once(ROOT_PATH . 'includes/cls_mysms.php');
    ...
    /* 检查有没有缺货 */
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('gift_card')." WHERE goods_id = '$goods[goods_id]' AND use_status=1 and card_status=1 and member_id=0 ";
    $num = $GLOBALS['db']->GetOne($sql);

    if ($num < $goods['num'])
    {
        $msg .= sprintf($GLOBALS['_LANG']['gift_card_oos'], $goods['goods_name']);

        return false;
    }

     /* 取出卡片信息 */
     $sql = "SELECT card_id, card_sn, card_password, end_date, crc32 FROM ".$GLOBALS['ecs']->table('gift_card')." WHERE goods_id = '$goods[goods_id]' AND use_status = 1 and member_id=0 and card_status=1 and end_date>".time()."  LIMIT " . $goods['num'];
     $arr = $GLOBALS['db']->getAll($sql);

     $card_ids = array();
     $cards = array();

     foreach ($arr as $gift_card)
     {
        $card_info = array();

        /* 卡号和密码解密 */
        if ($gift_card['crc32'] == 0 || $gift_card['crc32'] == crc32(AUTH_KEY))
        {
            $card_info['card_sn'] = $gift_card['card_sn'];
            $card_info['card_password'] = decrypt($gift_card['card_password']);
        }
        elseif ($gift_card['crc32'] == crc32(OLD_AUTH_KEY))
        {
            $card_info['card_sn'] = $gift_card['card_sn'];
            $card_info['card_password'] = decrypt($gift_card['card_password'], OLD_AUTH_KEY);
        }
        else
        {
            $msg .= 'error key';
        ...
        $card_info['end_date'] = date($GLOBALS['_CFG']['date_format'], $gift_card['end_date']);
        $card_ids[] = $gift_card['card_id'];
        $cards[] = $card_info;
     }
    ...
    /* 发送邮件 */
    $GLOBALS['smarty']->assign('gift_card',                   $cards);
    $GLOBALS['smarty']->assign('order',                          $order);
    $GLOBALS['smarty']->assign('goods',                          $goods);
    ...
    $tpl = get_mail_template('gift_card');
    $content = $GLOBALS['smarty']->fetch('str:' . $tpl['template_content']);
    send_mail($order['consignee'], $order['email'], $tpl['template_subject'], $content, $tpl['is_html']);
 ...
 * @return  boolen
 */
function phone_card_shipping ($goods, $order_sn, &$msg, $process = 'other')
{
    /* 包含加密解密函数所在文件 */
    include_once(ROOT_PATH . 'includes/lib_code.php');
    include_once(ROOT_PATH . 'includes/cls_mysms.php');
    ...
    /* 检查有没有缺货 */
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('phone_card')." WHERE goods_id = '$goods[goods_id]' AND use_status=1 and card_status=1 and member_id=0 ";
    $num = $GLOBALS['db']->GetOne($sql);

    if ($num < $goods['num'])
    {
        $msg .= sprintf($GLOBALS['_LANG']['phone_card_oos'], $goods['goods_name']);

        return false;
    }

     /* 取出卡片信息 */
     $sql = "SELECT card_id, card_sn, card_password, end_date, crc32 FROM ".$GLOBALS['ecs']->table('phone_card')." WHERE goods_id = '$goods[goods_id]' AND use_status = 1 and member_id=0 and card_status=1 and end_date>".time()."  LIMIT " . $goods['num'];
     $arr = $GLOBALS['db']->getAll($sql);

     $card_ids = array();
     $cards = array();

     foreach ($arr as $phone_card)
     {
        $card_info = array();

        /* 卡号和密码解密 */
        if ($phone_card['crc32'] == 0 || $phone_card['crc32'] == crc32(AUTH_KEY))
        {
            $card_info['card_sn'] = $phone_card['card_sn'];
            $card_info['card_password'] = decrypt($phone_card['card_password']);
        }
        elseif ($phone_card['crc32'] == crc32(OLD_AUTH_KEY))
        {
            $card_info['card_sn'] = $phone_card['card_sn'];
            $card_info['card_password'] = decrypt($phone_card['card_password'], OLD_AUTH_KEY);
        }
        else
        {
            $msg .= 'error key';
        ...
        $card_info['end_date'] = date($GLOBALS['_CFG']['date_format'], $phone_card['end_date']);
        $card_ids[] = $phone_card['card_id'];
        $cards[] = $card_info;
     }

     /* 标记已经取出的卡片 */
     $sql = "UPDATE ".$GLOBALS['ecs']->table('phone_card')." SET ".
           "use_status=2 ,".
           "order_sn = '$order_sn' ".
           "WHERE " . db_create_in($card_ids, 'card_id');
    if (!$GLOBALS['db']->query($sql, 'SILENT'))
    ...
    /* 发送邮件 */
    $GLOBALS['smarty']->assign('phone_card',                   $cards);
    $GLOBALS['smarty']->assign('order',                          $order);
    $GLOBALS['smarty']->assign('goods',                          $goods);
    ...
    $tpl = get_mail_template('phone_card');
    $content = $GLOBALS['smarty']->fetch('str:' . $tpl['template_content']);
    send_mail($order['consignee'], $order['email'], $tpl['template_subject'], $content, $tpl['is_html']);
 ...
 * @return void
 */
function gift_card_result($order_sn, $goods)
{
    /* 包含加密解密函数所在文件 */
//    include_once(ROOT_PATH . 'includes/lib_code.php');

    /* 获取已经发送的卡片数据 */
   $sql = "SELECT card_sn, card_password, end_date, crc32 FROM ".$GLOBALS['ecs']->table('gift_card')." WHERE goods_id= '$goods[goods_id]' AND order_sn = '$order_sn' ";
    $res= $GLOBALS['db']->query($sql);
    $cards = array();
 ...
 * @return void
 */
function phone_card_result($order_sn, $goods)
{
    /* 包含加密解密函数所在文件 */
//    include_once(ROOT_PATH . 'includes/lib_code.php');

    /* 获取已经发送的卡片数据 */
   $sql = "SELECT card_sn, card_password, end_date, crc32 FROM ".$GLOBALS['ecs']->table('phone_card')." WHERE goods_id= '$goods[goods_id]' AND order_sn = '$order_sn' ";
    $res= $GLOBALS['db']->query($sql);
    $cards = array();
...
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
{
    ...
    if ($user_money > 0)
    {
    	admin_log("user_id:$user_id;money:$user_money;", 'add', 'change_user_money');
    }
}
 ...
 */
function set_unsale(){
    $sql = "UPDATE ".$GLOBALS['ecs']->table('goods'). " SET is_on_sale = 0 WHERE extension_code = 'charge_calls'";
    $GLOBALS['db']->query($sql);
}
