 ...
 * @return  boolen      $result
 */
function add_gift($user_id, $gift_sn,$password)
{
	if (empty($user_id))
	{
		$GLOBALS['err']->add($GLOBALS['_LANG']['not_login']);
	...
	/* 查询红包序列号是否已经存在 */
	$sql = "SELECT card_id, card_sn, user_id,end_date,member_id,card_password FROM " .$GLOBALS['ecs']->table('gift_card') .
	//           " WHERE card_sn = '".$gift_sn."'";
	" WHERE card_sn = '".$gift_sn."' and card_password='".encrypt($password)."'";

	$row = $GLOBALS['db']->getRow($sql);
	//    echo decrypt($row['card_password']);exit;
	if ($row)
	{
		if ($row['member_id'] == 0)
		{
			...
			if ($now > $row['end_date'])
			{
				$GLOBALS['err']->add($GLOBALS['_LANG']['gift_use_expire']);
				return false;
			}

			$sql = "UPDATE " .$GLOBALS['ecs']->table('gift_card') . " SET member_id = '$user_id' ".
		...
		else
		{
			...
			{
				//礼品卡已经添加过了。
				$GLOBALS['err']->add($GLOBALS['_LANG']['gift_is_used']);
			}
			else
			{
				//礼品卡被其他人使用过了。
				$GLOBALS['err']->add($GLOBALS['_LANG']['gift_is_used_by_other']);
			}
	...
	{
		//红包不存在
		$GLOBALS['err']->add($GLOBALS['_LANG']['gift_not_exist']);
		return false;
	}

}

function merge_gift($user_id, $gift_sn,$merge_gift_sn)
{
	if (empty($user_id))
	{
		$GLOBALS['err']->add($GLOBALS['_LANG']['not_login']);
	...
	/* 查询红包序列号是否已经存在 */
	$sql = "SELECT card_id, card_sn, member_id,end_date,card_password,leave_money FROM " .$GLOBALS['ecs']->table('gift_card') ." WHERE card_sn = '".$gift_sn."'";
	$row = $GLOBALS['db']->getRow($sql);
	$sql2 = "SELECT card_id, card_sn, member_id,end_date,card_password,leave_money FROM " .$GLOBALS['ecs']->table('gift_card') ." WHERE card_sn = '".$merge_gift_sn."'";
	$row2 = $GLOBALS['db']->getRow($sql2);
	//    echo decrypt($row['card_password']);exit;
	if ($row && $row2)
	{
		if ($row['member_id'] == $user_id && $row2['member_id'] == $user_id)
		{
			//红包没有被使用
			$now = gmtime();
			if ($now > $row2['end_date'])
			{
				$GLOBALS['err']->add($GLOBALS['_LANG']['gift_use_expire']);
				return false;
			}
			if ($row['card_id'] == $row2['card_id'])
			{
				$GLOBALS['err']->add($GLOBALS['_LANG']['not_same']);
				return false;
			}

			$sql = "UPDATE " .$GLOBALS['ecs']->table('gift_card') . " SET leave_money = 0,card_status=4,use_status=3 "."WHERE card_id = '".$row[card_id]."'";
			$result = $GLOBALS['db'] ->query($sql);
			$leave_money = $row['leave_money']+$row2['leave_money'];
			$sql_m = "UPDATE " .$GLOBALS['ecs']->table('gift_card') . " SET leave_money = ".$leave_money." WHERE card_id = '".$row2[card_id]."'";

			$result = $GLOBALS['db'] ->query($sql_m);
		...
		{
			$GLOBALS['err']->add($GLOBALS['_LANG']['merge_gift_not_exist']);
...
function add_phone($user_id, $phone_sn,$password)
{
	if (empty($user_id))
	{
		$GLOBALS['err']->add($GLOBALS['_LANG']['not_login']);
	...
	/* 查询充值卡序列号是否已经存在 */
	$sql = "SELECT card_id, card_sn, user_id,member_id,end_date,card_password FROM " .$GLOBALS['ecs']->table('phone_card') .
	//	           " WHERE card_sn = '".$phone_sn."'";
	" WHERE card_sn = '".$phone_sn."' and card_password='".encrypt($password)."'";
	...
	if ($row)
	{
		...
		if ($row['member_id'] == 0)
		{
			...
			if ($now > $row['end_date'])
			{
				$GLOBALS['err']->add($GLOBALS['_LANG']['phone_use_expire']);
				return false;
			}

			$sql = "UPDATE " .$GLOBALS['ecs']->table('phone_card') . " SET member_id = '$user_id' ".
			"WHERE card_id = '".$row[card_id]."'";
			$result = $GLOBALS['db'] ->query($sql);
			if ($result)
			{
		...
		else
		{
			...
			{
				//礼品卡已经添加过了。
				$GLOBALS['err']->add($GLOBALS['_LANG']['phone_is_used']);
			}
			else
			{
				//礼品卡被其他人使用过了。
	...
	{
		//红包不存在
		$GLOBALS['err']->add($GLOBALS['_LANG']['phone_not_exist']);
		return false;
	}
}


function cz_phone($user_id, $phone_sn)
{
	if (empty($user_id))
	{
		$GLOBALS['err']->add($GLOBALS['_LANG']['not_login']);
	...
	/* 查询充值卡序列号是否已经存在 */
	$sql = "SELECT card_id, card_sn, user_id,member_id,end_date,card_password, leave_money FROM " .$GLOBALS['ecs']->table('phone_card') ." WHERE card_sn = '".$phone_sn."'";

	$row = $GLOBALS['db']->getRow($sql);
	if ($row)
	{
		if ($row['member_id']== $user_id && $row['leave_money']>0 && $row['end_date']>$now)
		{
			//充值操作。
			$sql = "INSERT INTO " .$GLOBALS['ecs']->table('user_account').
			" VALUES ('', '$user_id', '$user_id', ".$row['leave_money'].", '".gmtime()."', '".gmtime()."', '".$row['card_sn']."', '充值卡充值', '0', '充值卡充值', '1')";
			$GLOBALS['db']->query($sql);
			//充值
			$sql2 = "INSERT INTO " . $GLOBALS['ecs']->table('account_log') . "(user_id, user_money, change_time, change_desc, change_type)" . " VALUES ('{$user_id}', '{$row['leave_money']}', '". gmtime() ."', '充值卡充值', '99')";
			$GLOBALS['db']->query($sql2);
			$sql_u = "UPDATE ".$GLOBALS['ecs']->table('users') ." SET user_money= `user_money`+".$row['leave_money']."  WHERE user_id = '{$user_id}'";
			$GLOBALS['db']->query($sql_u);
			//			$valArr = array();
			//			$valArr['user_money'] = `user_money`+$row['leave_money'];
			//			update_user($user_id,$valArr);

			//更新充值卡状态
			$sql3 = "UPDATE ".$GLOBALS['ecs']->table('phone_card') ." SET leave_money= '0',use_status=3 WHERE card_sn = '{$row['card_sn']}'";
			$GLOBALS['db']->query($sql3);
			$GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['phone_succ_account'],$row['leave_money']));
		}
		else
		{
			//充值失败
			$GLOBALS['err']->add($GLOBALS['_LANG']['phone_fail_account']);
		}
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
	...
	$sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, pay_id, has_refunded, add_time, " .
	...
	while ($row = $GLOBALS['db']->fetchRow($res))
	{
        ...
        if($row['has_refunded'] == 0 && $row['pay_id'] == 10 && $row['pay_status'] == PS_PAYED){
            $row['refund'] = "<a href=\"user.php?act=suixing_refund&order_sn=" .$row['order_sn']. "\" onclick=\"if (!confirm('确定申请退款吗？')) return false;\">申请退款</a>";
        }
        else if ($row['has_refunded'] == 1 && $row['pay_id'] == 10){
            $row['refund'] = "<a href=\"user.php?act=refund_list&order_sn=" .$row['order_sn']. "\" >查看退款进度</a>";
        }
...
function get_order_detail($order_id, $user_id = 0)
{
	...
	$order['pay_mobile'] = $GLOBALS['db']->getOne("SELECT  mobile_phone FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '" . $_SESSION['user_id'] . "' ");
	//检查订单是否属于该用户
	...
	$order['order_status'] == OS_CONFIRMED))
	{
		...
		*/
		...
		else
		{
            ...
            if($payment_info['pay_code'] == 'telecompay'){
                $cart_goods = order_goods($order_id);
                $order['goods_name'] = $cart_goods[0]['goods_name'];
                $order['goods_number'] = $cart_goods[0]['goods_number'];
                $order['user_name'] = $_SESSION['user_name'];
            ...
            if($payment_info['pay_code'] == 'alipay'){
                $cart_goods = order_goods($order_id);
                $order['goods_name'] = $cart_goods[0]['goods_name'];
                $order['goods_category'] = get_goodscategory($cart_goods[0]['goods_id']);
                $order['user_name'] = $_SESSION['user_name'];
                $userinfo = get_userinfo($_SESSION['user_id']);
                $order['user_regtime'] = $userinfo["reg_time"];
                $order['user_phone'] = $userinfo["phone"];
	...
	if ($order['pay_status'] != PS_UNPAYED)
	{
		...
		foreach ($virtual_goods AS $code => $goods_list)
		{
			...
			if ($code == 'gift_card')
			{
				...
				foreach ($goods_list as $goods)
				{
					...
					if ($info = gift_card_result($order['order_sn'], $goods))
					{
				
						$gift_card[] = array('goods_id'=>$goods['goods_id'], 'goods_name'=>$goods['goods_name'], 'info'=>$info);
					}
				}
			}
			if ($code == 'phone_card')
			{
				foreach ($goods_list as $goods)
				{
					if ($info = phone_card_result($order['order_sn'], $goods))
					{
						$phone_card[] = array('goods_id'=>$goods['goods_id'], 'goods_name'=>$goods['goods_name'], 'info'=>$info);
					}
				}
			}
			/* 处理超值礼包里面的虚拟卡 */
		...
		$GLOBALS['smarty']->assign('phone_card', $phone_card);
		$GLOBALS['smarty']->assign('gift_card', $gift_card);
	}
 ...
 * @param   int         $start           显示起始位置
 *
 * @return  array       $arr             礼品卡列表
 */
function get_user_gift_list($user_id, $num = 10, $start = 0)
{
	$sql = "SELECT b.card_sn, b.order_id, u.goods_name, b.money, b.leave_money,  b.end_date,b.card_status,b.use_status,b.card_password ".
	" FROM " .$GLOBALS['ecs']->table('gift_card'). " AS b ,".
	$GLOBALS['ecs']->table('goods'). " AS u".
	" WHERE u.goods_id = b.goods_id AND b.member_id = '" .$user_id. "'";
	$res = $GLOBALS['db']->selectLimit($sql, $num, $start);
	$arr = array();
	...
	while ($row = $GLOBALS['db']->fetchRow($res))
	{
		...
		if (empty($row['order_sn']))
		{
			...
			else
			{
				...
				$row['card_status'] = $GLOBALS['_LANG']['card_status'][$cardStatus];
				$useStatus = $row['use_status'];
				$row['use_status'] = $GLOBALS['_LANG']['use_status'][$useStatus];

			}
		}
		else
		{
			$row['status'] = '<a href="user.php?act=order_detail&order_id=' .$row['order_sn']. '" >' .$GLOBALS['_LANG']['had_use']. '</a>';
		}
...
function get_user_phone_list($user_id, $num = 10, $start = 0)
{
	$sql = "SELECT b.card_sn, b.order_sn, u.goods_name, b.money, b.leave_money,  b.end_date,b.card_status,b.use_status,b.card_password,b.member_id ".
	" FROM " .$GLOBALS['ecs']->table('phone_card'). " AS b ,".
	$GLOBALS['ecs']->table('goods'). " AS u".
	" WHERE u.goods_id = b.goods_id AND b.member_id = '" .$user_id. "'";
	$res = $GLOBALS['db']->selectLimit($sql, $num, $start);
	$arr = array();
 ...
 * @return  array       $order_list     订单列表
 */
function get_user_refundlist($user_id,$order_sn, $num = 10, $start = 0)
{
    /* 取得订单列表 */
    $arr    = array();
    ...
    $sql = "SELECT order_sn, refordno, status, add_time " .
        " FROM " .$GLOBALS['ecs']->table('refund_info') .
    ...
    {
        if($row['status'] == 1){
            $row['refund_status'] = "处理中";
        }
        else if($row['status'] == 3){
            $row['refund_status'] = "审核通过，额度已退您白条账号";
        }
        else{
            $row['refund_status'] = "审核不通过  请联系客服4000851115";
        }
        $arr[] = array('order_sn'       => $row['order_sn'],
            'refordno'       => $row['refordno'],
            'order_time'     => local_date($GLOBALS['_CFG']['time_format'], $row['add_time']),
 ...
 * @return  int
 */
function insert_suixing_refund($order_sn, $refordno, $userid, $username, $refund_reason)
{
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('refund_info').
        ' (order_sn, refordno, userid, username, status, refund_reason, action_user, add_time, refund_time)'.
        " VALUES ('$order_sn', '$refordno', '$userid', '$username', 1, '$refund_reason', null, '".gmtime()."', null)";
    $GLOBALS['db']->query($sql);
    $id = $GLOBALS['db']->insert_id();

    if($GLOBALS['db']->insert_id() > 0){
        $sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_info') . " SET has_refunded='1' WHERE order_sn = '$order_sn'";
        $result = $GLOBALS['db']->query($sql);
    }
    if($result && $id > 0){
        return true;
