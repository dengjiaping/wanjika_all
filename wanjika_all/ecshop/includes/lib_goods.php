...
function get_categories_tree($cat_id = 0)
{
    ...
    $all_sql = 'SELECT cat_id, cat_name, parent_id, is_show ' .
        'FROM ' . $GLOBALS['ecs']->table('category') .
        "WHERE is_show = 1 ORDER BY sort_order ASC, cat_id ASC";
    $result = $GLOBALS['db']->getAll($all_sql);

    //声明以parent_id为key值的数组变量
    foreach ($result AS $row)
    {
        if (!isset($cat_all[$row['parent_id']]))
        {
            $cat_all[$row['parent_id']] = array();
        }
        if ($row['is_show'])
        {
            array_push($cat_all[$row['parent_id']],$row);
        }
    }
    ...
    */
    ...
    if (count($cat_all[$parent_id]) || $parent_id == 0)
    {
        /* 获取当前分类及其子分类 */
...
//        $res = $GLOBALS['db']->getAll($sql);
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                ...
                {
                    $cat_arr[$row['cat_id']]['cat_id'] = get_child_tree($row['cat_id'],$cat_all);
                }
            }
        }
    }
...
function get_child_tree($tree_id = 0,$cat_all = array())
{
    $three_arr = array();
    $count = 0;
//    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$tree_id' AND is_show = 1 ";
    if(isset($cat_all[$tree_id])){
        $count = count($cat_all[$tree_id]);
    }
...
//        $res = $GLOBALS['db']->getAll($child_sql);
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
               ...
               if (isset($row['cat_id']) != NULL)
                   {
                       $three_arr[$row['cat_id']]['cat_id'] = get_child_tree($row['cat_id'],$cat_all);
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_cuxiao, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['cuxiao']         = $row['goods_cuxiao'];
 ...
 * @return  array
 */
function get_charge_goods()
{
    $sql = "SELECT  goods_id, keywords FROM " . $GLOBALS['ecs']->table('goods') .
        " WHERE extension_code = 'charge_calls' and is_on_sale = 2 and is_delete = 0";
    $res = $GLOBALS['db']->getAll($sql);

    $charge_goods = array();
    foreach($res AS $val){
        $charge_goods[$val['goods_id']] = $val['keywords'];
    }
    asort($charge_goods);
 ...
 * @return  array
 */
function get_indexpromote_goods($cats = '')
{
    $time = gmtime();
    $order_type = $GLOBALS['_CFG']['recommend_order'];
    ...
    $sql = 'SELECT g.goods_id, g.goods_cuxiao, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
        ...
        'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' .
        " AND g.is_promote = 1 AND promote_start_date <= '$time' AND promote_end_date >= '$time' ";
    $sql .= $order_type == 0 ? ' ORDER BY g.last_update DESC' : ' ORDER BY rnd';
    $sql .= " LIMIT $num ";
    $result = $GLOBALS['db']->getAll($sql);
 ...
 * @return  array
 */
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '', $flag = true)
{
    $children = get_children($cat_id);

    $sql = 'SELECT g.goods_id, g.goods_cuxiao, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
