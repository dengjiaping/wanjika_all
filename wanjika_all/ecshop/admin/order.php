...
if ($_REQUEST['act'] == 'order_query')
{
    ...
    /* 载入用户等级 */
    $smarty->assign('rank_list', get_rank_list());
	...
	$smarty->assign('rs_list', get_referers());
...
if ($_REQUEST['act'] == 'patch_order_by_mobile')
{
	if ($_POST['orderlist']!="") {
		$moblist = explode("\n",$_POST['mobile_list']);
		$smarty->assign('posted',    1);
		if (!$_FILES['order_file']['size']) {
			$smarty->assign('show',    "请上传文件");
		}
		else {
			$uploaddir = '/web/ecshop/upload_order';
			$uploadfile = $uploaddir."/".time(). $_FILES['order_file']['name'];
			if (move_uploaded_file($_FILES['order_file']['tmp_name'], $uploadfile))
			{
				$sql = "INSERT INTO ".$ecs->table('patch_order_file')." (src_file, dest_file, user_id, add_time ) ".
				"VALUES ('".$_FILES['order_file']['name']."','".$uploadfile."','".$_SESSION['user_id']."','".time()."') ";
				$res = $db->query($sql);
				$show = "文件上传成功！(系统默认5分钟执行一次)<br>其中异常数据如下：";
			}
			else {
				$show = "文件上传失败！<br>";
			}
			...
			if (is_file($uploadfile)) {
				$orderlist = file($uploadfile);
			}
			while (list($key,$srcInfo)=each($orderlist)) {
				$srcArr = explode(",",$srcInfo);

				$mobile = $srcArr[0];
				$adress  = mb_convert_encoding($srcArr[1],'utf-8','gbk');
				$gdID = $srcArr[2];
				$gdNum = $srcArr[3];
				$gdSql = "SELECT goods_sn,goods_id FROM " . $ecs->table('goods') . " WHERE goods_id = '".$gdID."' or goods_sn='".$gdID."'";
				$gdRes = $db->getOne($gdSql);
				if ($gdRes['goods_id'] || $gdNum>0 )
				{
					$isGood=1;
				}
				if(preg_match("/1\d{10}/",$mobile) && $isGood)
				{
					$sql = "INSERT INTO ".$ecs->table('patch_order')." (mobile, adress, goods_id, goods_num ) ".
					"VALUES ('".$mobile."','".$adress."','".$gdID."','".$gdNum."') ";
					$res = $db->query($sql);
				}
				else {
					$show.="<br>".mb_convert_encoding($srcInfo,'utf-8','gbk')." | 非法订单";
				}
			}
			$smarty->assign('show',    $show);
		}
	}
	$smarty->display('patch_order_by_mobile.htm');
...
elseif ($_REQUEST['act'] == 'list')
{
	...
	if ($_REQUEST['download']=='下载') {
		...
		$str_down = "订单号\t合并订单号\t商品名称\t下单时间\t地址\t电话\t手机\t最佳送货时间\t收货人\t商品数量\t折扣\t发票税额\t配送费用\t保价费用\t支付费用\t包装费用\t贺卡费用\t商品总金额\t已付款金额\t订单状态\t发货状态\t支付状态\t支付方式\t支付源\t是否海淘\t分类";
...
elseif ($_REQUEST['act'] == 'refundlist')
{
    /* 检查权限 */
    admin_priv('order_view');
    /* 模板赋值 */
    $smarty->assign('full_page',1);
    $list = refund_list();
    $smarty->assign('ur_here', $_LANG['12_suixin_refund']);
    $smarty->assign('refund_list',   $list['refundlist']);
...
elseif ($_REQUEST['act'] == 'step_post')
{
	...
	elseif ('payment' == $step)
	{
        ...
        $order = order_info($order_id);
        $mobile = $db->getOne("SELECT  mobile_phone FROM " .$ecs->table('users'). " WHERE user_id = '" . $order['user_id'] . "' ");
		if ($payment['is_cod'] == 1)
		{
			$region_id_list = array(
			$order['country'], $order['province'], $order['city'], $order['district']
			...
			$pay_fee = pay_fee($pay_id, $order_amount, $shipping['pay_fee'], $mobile);
		}
		else
		{
			$pay_fee = pay_fee($pay_id, $order_amount, 0, $mobile);
		}
	...
	elseif ('money' == $step)
	{
        ...
        //订单金额四舍五入到毛
        $order['order_amount']  = number_format($order['amount'], 1, '.', '');
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
	...
	elseif ('payment' == $step)
	{
		...
		if (exist_real_goods($order_id))
		{
			...
			$payment_list = available_payment_list_by_goods($goods_ids, $shipping_area['support_cod'], $pay_fee);
		}
		else
		{
			/* 不存在实体商品 */
			...
			$payment_list = available_payment_list_by_goods($goods_ids, false);
		}
	...
	elseif ('money' == $step)
	{
		...
		if ($order['user_id'] > 0)
		{
			...
			$goods_list = order_goods($order_id);
			$user_bonus = user_available_bonus($order['user_id'], $goods_list);
			if ($order['bonus_id'] > 0)
			{
...
//-- 闪白条退款操作
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'suixing_refund')
{
    /* 检查权限 */
    check_authz_json('order_edit');
    ...
    if ($id == 0)
    {
        $links[] = array('text' => $_LANG['12_suixin_refund'], 'href' => 'order.php?act=refundlist&status=1');
        sys_msg('操作失败 Id不能为空', 0, $links);
    }

    include_once(ROOT_PATH . 'includes/modules/payment/suixingpay.php');
    $sql = "SELECT * FROM ecs_refund_info WHERE id = '" . $id . "'";
    $result = $GLOBALS['db']->getAll($sql);
    if(count($result) <= 0){
        $links[] = array('text' => $_LANG['12_suixin_refund'], 'href' => 'order.php?act=refundlist&status=1');
        sys_msg('获取退款申请信息错误，退款申请失败！', 0, $links);
    }
    $sql = "SELECT money_paid FROM ecs_order_info WHERE order_sn = '" . $result[0]['order_sn'] . "'";
    $money_paid = $GLOBALS['db']->getOne($sql);

    $pay_obj = new suixingpay();
    $r = $pay_obj->refund($result[0]['order_sn'], $result[0]['refordno'], $money_paid, $id);
    if ($r)
    {
        $links[] = array('text' => $_LANG['12_suixin_refund'], 'href' => 'order.php?act=refundlist&status=1');
...
//-- 拒绝闪白条退款申请
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'refuse_refund')
{
    /* 检查权限 */
    check_authz_json('order_edit');
    ...
    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('refund_info') . " SET status='2', action_user = '$action_user' WHERE id = '$id'";
    if ($GLOBALS['db']->query($sql))
    {
        $links[] = array('text' => $_LANG['12_suixin_refund'], 'href' => 'order.php?act=refundlist&status=1');
    ...
    {
        $links[] = array('text' => $_LANG['12_suixin_refund'], 'href' => 'order.php?act=refundlist&status=1');
        sys_msg($GLOBALS['db']->errorMsg(), 0, $links);
    }
}
...
function order_list()
{
	...
	if ($result === false)
	{
        ...
        $arr_id = explode(",",$filter['goods_id']);
        $flag = count($arr_id) > 1 ? 1 :0;
        ...
        $filter['is_charge'] = empty($_REQUEST['is_charge']) ? 0 : intval($_REQUEST['is_charge']);
        ...
        //只显示2015年以后订单
//        if($filter['start_time'] < 1420012800){
//            $filter['start_time'] = 1420012800;
//        }
//        if(!empty($filter['end_time']) && $filter['end_time'] < 1420012800){
//            $filter['end_time'] = 1420012800;
//        }
        ...
        if ($filter['is_charge'])
        {
            $where .= " AND g.extension_code = 'charge_calls'";
        }
        if ($filter['goods_id'])
        {
            if($flag){
                $where .= " AND g.goods_id IN ($filter[goods_id])";
            }
            else{
		...
		/* 记录总数 */
		if ($filter['user_name'] || $filter['user_rank'] || $filter['goods_id'] || $filter['is_charge'])
		{
...
//        if ($filter['goods_id'])
//        {
//            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_goods') . " AS g ". $where;
//        }
		...
		/* 查询 */
		$sql = "SELECT o.order_id, o.order_sn, o.country, u.mobile_phone, o.province, o.city, o.district, o.add_time, o.address, o.mobile,o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
		"o.pay_status, o.pay_source, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id, o.pay_name, g.goods_number, g.goods_price, " .
        "o.discount,o.tax,o.shipping_fee,o.insure_fee,o.pay_fee,o.pack_fee,o.card_fee,o.best_time,c.cat_name,gs.is_overseas," .
		...
		" LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS go ON g.goods_id=go.goods_id " .
		" LEFT JOIN " . $GLOBALS['ecs']->table('goods_supplier') . " gs ON g.supplier_id=gs.type_id " .
		" LEFT JOIN " . $GLOBALS['ecs']->table('category') . " AS c ON go.cat_id=c.cat_id " . $where .
...
function refund_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['user_name'])
        {
            $where .= " AND username LIKE '%" . mysql_like_quote($filter['user_name']) . "%'";
        }
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('refund_info'). $where;

        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;

        /* 查询 */
        $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('refund_info') .  $where .
            " ORDER BY $filter[sort_by] $filter[sort_order] ".
            " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
    ...
    {
        $row[$key]['short_order_time'] = local_date('m-d H:i', $value['add_time']);
        $row[$key]['short_refund_time'] = local_date('m-d H:i', $value['refund_time']);
        $row[$key]['refund_reason'] = substr($row[$key]['refund_reason'],0,240).'...';
    }
    $arr = array('refundlist' => $row, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
