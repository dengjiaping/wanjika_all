...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    $handler_list['phone_card'][] = array('url'=>'phone_card.php?act=card', 'title'=>$_LANG['phone_card'], 'img'=>'icon_send_bonus.gif');
    $handler_list['phone_card'][] = array('url'=>'phone_card.php?act=replenish', 'title'=>$_LANG['replenish'], 'img'=>'icon_add.gif');
    $handler_list['phone_card'][] = array('url'=>'phone_card.php?act=batch_card_add', 'title'=>$_LANG['batch_card_add'], 'img'=>'icon_output.gif');
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    {
        $putaway_goods = admin_priv('putaway_goods',"",false) ? 1 : 2;
        $smarty->assign('putaway_goods', $putaway_goods);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    /* 处理商品数据 */
    $keywords = !empty($_POST['charge_amount']) ? $_POST['charge_amount'] : '';
    ...
    if(trim($_REQUEST['extension_code']) == 'charge_calls'){
        $is_on_sale = 2;
        $_POST['keywords'] = $_POST['charge_amount'];
    }
    ...
    $goods_cuxiao = !empty($_POST['goods_cuxiao']) ? $_POST['goods_cuxiao'] : '';
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id, payment_ids, goods_cuxiao)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id', '$payments_ids', '$goods_cuxiao')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral, payment_ids, goods_cuxiao)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral', '$payments_ids', '$goods_cuxiao')";
        }
    }
    else
    {
...
//-- 修改促销语
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_goods_cuxiao')
{
    check_authz_json('goods_manage');

    $goods_id   = intval($_POST['id']);
    $goods_cuxiao = json_str_iconv(trim($_POST['val']));

    if ($exc->edit("goods_cuxiao = '$goods_cuxiao', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result(stripslashes($goods_cuxiao));
    }
}
...
{
	// 导出到文本;
	$file_name = "goods_list_". date ( "Ymd" ) . ".csv";
	$str_down = "编号,商品名称,货号,价格,上架,新品,精品,热销,库存,分类";
	$str_down = mb_convert_encoding ( $str_down, "gb2312", "UTF-8" );
	
	$is_delete = empty($_REQUEST['is_delete']) ? 0 : intval($_REQUEST['is_delete']);
    $code = empty($_REQUEST['extension_code']) ? '' : trim($_REQUEST['extension_code']);
    $goods_list = download_goods_list();

	foreach ($goods_list as $key => $val) {
		$str_down .= "\n" . $val ['goods_id'] . "," . mb_convert_encoding ( $val['goods_name'], "gb2312", "UTF-8" ) . ',' . mb_convert_encoding($val['goods_sn'], "gb2312", "UTF-8") . "," . $val['shop_price'] . "," . mb_convert_encoding(($val ['is_on_sale'] ? '是' : '否'), "gb2312", "UTF-8") . "," . mb_convert_encoding(($val ['is_new'] ? '是' : '否'), "gb2312", "UTF-8") . "," . mb_convert_encoding(($val ['is_best'] ? '是' : '否'), "gb2312", "UTF-8") . "," . mb_convert_encoding(($val ['is_hot'] ? '是' : '否'), "gb2312", "UTF-8") . "," . $val['goods_number'] . ',' . mb_convert_encoding ( $val['cat_name'], "gb2312", "UTF-8" );
	}
...
function download_goods_list()
{
	$day = getdate();
	$today = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
	$where = "1=1";
	$where .= $_REQUEST['cat_id'] > 0 ? " AND " . get_children($_REQUEST['cat_id']) : '';
	/* 推荐类型 */
	switch ($_POST['intro_type'])
	{
		case 'is_best':
			$where .= " AND is_best=1";
			break;
		...
		/* 扩展 */
		if ($_REQUEST['extension_code'])
		{
			$where .= " AND extension_code='$_REQUEST[extension_code]'";
		}
        ...
        {
            $where .= " AND (suppliers_id = '" . $_REQUEST['suppliers_id'] . "')";
        }
	
	$sql = "SELECT g.goods_id, g.goods_name, g.goods_sn, g.shop_price, g.is_on_sale, g.is_best, g.is_new, g.is_hot, g.goods_number, t.cat_name, " .
					" (g.promote_price > 0 AND g.promote_start_date <= '$today' AND g.promote_end_date >= '$today') AS is_promote ".
                    " FROM " . $GLOBALS['ecs']->table('goods') . " AS g LEFT JOIN " . $GLOBALS['ecs']->table('category') . " AS t " . 
