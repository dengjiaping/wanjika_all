...
if ($_REQUEST['act'] == 'list')
{
    ...
    $start_date = empty($_REQUEST['start_date']) ? '' : (strpos($_REQUEST['start_date'], '-') > 0 ?  local_strtotime($_REQUEST['start_date']) : $_REQUEST['start_date']);
    $end_date = empty($_REQUEST['end_date']) ? '' : (strpos($_REQUEST['end_date'], '-') > 0 ?  local_strtotime($_REQUEST['end_date']) : $_REQUEST['end_date']);
    ...
    $sql = "SELECT COUNT(*) AS total_order_num, " .$total_fee.
           ...
           " WHERE 1 " . order_query_sql('confirmed_paid');
