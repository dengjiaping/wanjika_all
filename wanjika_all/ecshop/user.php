...
include(dirname(__FILE__) . '/includes/set_adheader.php');
/* 载入语言文件 */
...
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'gift', 'phone', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer', 'suixing_refund', 'refund_submit','refund_list');
...
elseif ($action == 'act_add_gift')
{
    ...
    {
        show_message($_LANG['add_gift_sucess'], $_LANG['back_up_page'], 'user.php?act=gift', 'info');
    }
    else
    {
        $err->show($_LANG['back_up_page'], 'user.php?act=gift');
    }
}
...
//    print_r($_POST);
    ...
    {
        show_message($_LANG['merge_gift_sucess'], $_LANG['back_up_page'], 'user.php?act=gift', 'info');
    }
    else
    {
        $err->show($_LANG['back_up_page'], 'user.php?act=gift');
...
/* 添加一个充值卡 */
elseif ($action == 'act_add_phone')
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');

    $phone_sn = $_POST['phone_sn'];
    $phone_password = $_POST['phone_password'];
    if (add_phone($user_id, $phone_sn,$phone_password))
    {
        show_message($_LANG['add_phone_sucess'], $_LANG['back_up_page'], 'user.php?act=phone', 'info');
    }
    else
    {
        $err->show($_LANG['back_up_page'], 'user.php?act=phone');
    }
}

/* 充值到个人账户 */
elseif ($action == 'act_cz_phone')
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');

    $phone_sn = $_POST['phone_sn'];
    if (cz_phone($user_id,$phone_sn))
    {
        show_message(sprintf($_LANG['cz_phone_sucess'], price_format($phone_sn, false)), $_LANG['back_up_page'], 'user.php?act=phone', 'info');
    }
    else
    {
        $err->show($_LANG['back_up_page'], 'user.php?act=phone');
...
elseif ($action == 'refund_list')
{
    ...
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('refund_info'). " WHERE userid = '$user_id'");

    $pager  = get_pager('user.php', array('act' => $action), $record_count, $page);

    $refundlist = get_user_refundlist($user_id,$order_sn, $pager['size'], $pager['start']);

    $smarty->assign('pager',  $pager);
    $smarty->assign('refundlist', $refundlist);
    $smarty->display('user_transaction.dwt');
}

/* 闪白条退款申请界面 */
elseif ($action == 'suixing_refund')
{
    $smarty->assign('order_sn', $_REQUEST['order_sn']);
    $smarty->display('user_transaction.dwt');
}

elseif ($action == 'refund_submit')
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
    $order_sn = isset($_POST['order_sn']) ? $_POST['order_sn'] : "";
    $refund_reason = isset($_POST['refund_reason']) ? $_POST['refund_reason'] : "";
    if ($order_sn == "" || $refund_reason == "")
    {
        show_message("请输入退款理由！");
    }

    $refordno = $order_sn.$_SESSION['user_id'];
    //插入退款申请
    $r = insert_suixing_refund($order_sn,$refordno,$_SESSION['user_id'],$_SESSION['user_name'], $refund_reason);

    /* 如果成功提交 */
    if ($r)
    {
        show_message("退款申请提交成功！", $_LANG['label_refund'], 'user.php?act=refund_list', 'info');
    }
    else
    {
        show_message("退款申请提交失败！", $_LANG['back_page_up'], '', 'info');
    }
}
...
elseif ($action == 'account_deposit')
{
    ...
    //余额充值方式去掉
    $list = get_online_payment_list(false);
    foreach($list as $k=>$v){
        if($v["pay_id"] == 3){
            unset($list[$k]);
        }
        if ($v['pay_code'] == 'mobpay')
        {
            $list[$k]['pay_fee'] = '8%';
        }
    }
    $smarty->assign('payment', $list);
...
elseif ($action == 'act_account')
{
    ...
    else
    {
        ...
        $order['order_id']       = $surplus['rec_id'];
        $order['user_name']      = $_SESSION['user_name'];
        $order['surplus_amount'] = $amount;

        //计算支付手续费用
        $mobile = $db->getOne("SELECT  mobile_phone FROM " .$ecs->table('users'). " WHERE user_id = '" . $_SESSION['user_id'] . "' ");
        $payment_info['pay_fee'] = pay_fee($surplus['payment_id'], $order['surplus_amount'], 0, $mobile);
        if ($payment_info['pay_code'] == 'mobpay')
        {
            $payment_info['pay_fee'] = round($order['surplus_amount'] * 0.08, 1);
        }
        $order['pay_mobile'] = $mobile;
        $order['tel'] = $mobile;
...
elseif ($action == 'pay')
{
    ...
    if (!empty($payment_info))
    {
        ...
        //计算支付手续费用
        $mobile = $db->getOne("SELECT  mobile_phone FROM " .$ecs->table('users'). " WHERE user_id = '" . $_SESSION['user_id'] . "' ");
        $payment_info['pay_fee'] = pay_fee($payment_id, $order['surplus_amount'], 0, $mobile);
        $order['pay_mobile'] = $mobile;
        $order['tel'] = $mobile;
        //计算此次预付款需要支付的总金额
...
elseif ($action == 'act_edit_surplus')
{
    ...
    if ($order['order_amount'] > 0)
    {
        ...
        {
            $mobile = $db->getOne("SELECT  mobile_phone FROM " .$ecs->table('users'). " WHERE user_id = '" . $_SESSION['user_id'] . "' ");
            $pay_fee = pay_fee($order['pay_id'], $order['order_amount'], $cod_fee, $mobile);
        }
...
elseif ($action == 'phone')
{
    ...
    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('phone_card'). " WHERE member_id = '$user_id'");

    $pager = get_pager('user.php', array('act' => $action), $record_count, $page);
    $phone = get_user_phone_list($user_id, $pager['size'], $pager['start']);
    $smarty->assign('pager', $pager);
