...
elseif ($act == 'act_register')
{
    if($_CFG['shop_reg_closed']){
        ...
        {
            show_regmessage($_LANG['passport_js']['username_shorter']);
        }

        if (strlen($password) < 6)
        {
            show_regmessage($_LANG['passport_js']['password_shorter']);
        }

        if (strpos($password, ' ') > 0)
        {
            show_regmessage($_LANG['passwd_balnk']);
        }
        ...
        if (m_register($username, $password, $email, $other) !== false)
        {
            ...
            show_regmessage('注册成功！', '', 'index.php');
        }
        else
        {
