...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id,g.goods_cuxiao, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img, g.goods_number ' .
    ...
    foreach ($res as $row)
    {
        ...
        if ($watermark_img != '')
        {
            $tmp_arr['watermark_img'] =  $watermark_img;
        }

        $tmp_arr['goods_id']         = $row['goods_id'];
        if($display == 'grid')
        {
            $tmp_arr['goods_name']       = $GLOBALS['_CFG']['goods_name_length'] > 0 ? sub_str($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
        }
        else
        {
           $tmp_arr['goods_name']       = $row['goods_name'];
        }
        $tmp_arr['name']             = $row['goods_name'];
        $tmp_arr['cuxiao']             = $row['goods_cuxiao'];
        $tmp_arr['goods_brief']      = $row['goods_brief'];
        $tmp_arr['goods_style_name'] = add_style($row['goods_name'],$row['goods_name_style']);
        $tmp_arr['market_price']     = price_format($row['market_price']);
        $tmp_arr['shop_price']       = price_format($row['shop_price']);
        $tmp_arr['type']             = $row['goods_type'];
        $tmp_arr['promote_price']    = ($promote_price > 0) ? price_format($promote_price) : '';
        $tmp_arr['goods_thumb']      = get_image_path($row['goods_id'], $row['goods_thumb'], true);
        $tmp_arr['goods_img']        = get_image_path($row['goods_id'], $row['goods_img']);
		$tmp_arr['url']              = build_uri('goods', array('gid'=>$row['goods_id']), $row['goods_name']);
		$tmp_arr['goods_number'] = $row['goods_number'];

        if ($row['goods_number'] >0)
        {
            $arr_available[$row['goods_id']] = $tmp_arr;
        }
        else
        {
            $arr_not_available[$row['goods_id']] = $tmp_arr;
        }

    }

    $arr = $arr_available + $arr_not_available;

    $arr = array_slice($arr, ($page - 1) * $size, $size, true);
