...
function WCreateOauthUrlForCode($redirectUrl)
{
    $urlObj["appid"] = "wxf704c28f4562f8c5";
    $urlObj["redirect_uri"] = "$redirectUrl";
    $urlObj["response_type"] = "code";
    $urlObj["scope"] = "snsapi_userinfo";
    $urlObj["state"] = "STATE"."#wechat_redirect";
    $bizString = WToUrlParams($urlObj);
    return "https://open.weixin.qq.com/connect/oauth2/authorize?".$bizString;
}

function WCreateOauthUrlForOpenid($code)
{
    $urlObj["appid"] = "wxf704c28f4562f8c5";
    $urlObj["secret"] = "476bcbbfbaa50874913f8823df7dab7e";
    $urlObj["code"] = $code;
...
function wgetUserinfoFromMp($code)
{
    $url = WCreateOauthUrlForOpenid($code);
    //初始化curl
    $ch = curl_init();
    //设置超时
...
//    }
    ...
    $userinfo = wgetUserinfo($data['access_token'],$data['openid']);

    return $userinfo;
}

function wgetUserinfo($access_token,$openid)
{
    ...
    foreach($data as $key=>$val){
        //$data[$key] = mb_convert_encoding($val, 'gbk',mb_detect_encoding($val));
    }
