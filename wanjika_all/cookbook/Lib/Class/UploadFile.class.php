...
class UploadFile {//类定义开始
    ...
    private function save($file) {
        ...
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
				...
				$thumbComperssion =$this->thumbComperssion;//★缩略图质量
                // 生成图像缩略图
                ...
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
                    ...
                    $thumbname = Image::thumb($filename,$thumbPath.$thumbname.'.'.$thumbExt,'',$thumbWidth[$i],$thumbHeight[$i],true,$thumbComperssion);//★增加了缩略图质量
                }
    ...
    private function check($file) {
        ...
        if(!$this->checkSize($file['size'])) {
            $this->error = '请调低拍摄像素或者裁剪，图片小于6MB！';
            return false;
        }
